...
from server import send_booking_confirmation_email, send_via_gmail_api, EMAIL_TRANSLATIONS
...
def test_email_functions():
    ...
    # Test 2: Test Google Workspace Gmail API function directly
    print("\n2. Testing Google Workspace Gmail API Email Function...")
    try:
        result = send_via_gmail_api(test_booking)
        if result:
            print("✅ Gmail API email function executed successfully")
        else:
            print("⚠️ Gmail API returned False (check GOOGLE_SERVICE_ACCOUNT_JSON and domain-wide delegation)")
    except Exception as e:
        print(f"❌ Gmail API email function error: {str(e)}")
    ...
    for lang in languages:
        ...
        try:
            result = send_via_gmail_api(test_booking_lang)
