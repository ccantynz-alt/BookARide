...
export const AdminLogin = () => {
  ...
  useEffect(() => {
    ...
    const initGoogleSignIn = async () => {
      try {
        // Fetch client ID from backend (avoids needing VITE_ env var)
        const res = await axios.get(`${API}/admin/google-auth/client-id`);
        if (cancelled) return;
        googleClientIdRef.current = res.data.client_id;
        ...
        // Google sign-in not available — redirect fallback remains
      }
    };
    ...
    if (gisReady && window.google?.accounts?.id) {
      ...
      window.google.accounts.id.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          ...
          setGoogleLoading(false);
          window.location.href = `${BACKEND_URL || RENDER_BACKEND_URL}/api/admin/google-auth/start`;
        }
      });
    } else {
      // GIS not loaded — fall back to redirect flow through Render
      window.location.href = `${BACKEND_URL || RENDER_BACKEND_URL}/api/admin/google-auth/start`;
    }
  };
      ...
      <Card className="w-full max-w-md border-2 border-gold/30">
        <CardContent className="p-8">
          ...
          <button
            ...
            className="flex items-center justify-center w-full gap-2 py-3 px-4 border-2 border-gray-300 rounded-md bg-white hover:bg-gray-50 hover:border-gray-400 transition-colors font-medium text-gray-700 shadow-sm cursor-pointer disabled:opacity-50"
          >
            {googleLoading ? (
