...
export const BookNow = () => {
  ...
  const calculatePrice = async () => {
    ...
    try {
      ...
      // Backend returns subtotal/stripeFee/totalPrice (one-way only)
      const subtotal = response.data.subtotal ?? 0;
      const stripeFeeRaw = response.data.stripeFee != null ? response.data.stripeFee : ((subtotal * 0.029) + 0.30);
      const stripeFee = Math.round(stripeFeeRaw * 100) / 100;
      const totalPriceRaw = response.data.totalPrice != null ? response.data.totalPrice : (subtotal + stripeFee);
                ...
                <div className="lg:col-span-2 space-y-6">
                  <Card className="border-2 border-gray-200 shadow-lg">
                    <CardContent className="p-8">
                      ...
                      {/* One-way only: return trip section removed for a cleaner booking form */}
                    </CardContent>
                  </Card>
