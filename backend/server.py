...
def run_async_task(coro_func, arg, task_description="background task"):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Ã¢Å“â€¦ Background task completed: {task_description}")
        finally:
            loop.close()
    except Exception as e:
        logger.error(f"Ã¢ÂÅ’ Background task failed ({task_description}): {str(e)}")
...
def validate_booking_date(self):       
    ...
    if self.date:  # Ã¢Å“â€¦ NOW PROPERLY INSIDE THE FUNCTION
        try:
            nz_tz = pytz.timezone('Pacific/Auckland')
            today = datetime.now(nz_tz).strftime('%Y-%m-%d')
            if self.date < today:
            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <div class="warning">
                            Ã¢ÂÂ° <strong>This link will expire in 1 hour.</strong><br>
    ...
    try:
        ...
        if google_api_key:
            ...
            if len(all_pickups) > 1:
                ...
                if data['status'] == 'OK' and len(data['routes']) > 0:
                    ...
                    logger.info(f"Multi-stop route: {len(all_pickups)} pickups Ã¢â€ â€™ dropoff, total: {distance_km}km")
                else:
        ...
        if is_concert_trip:
            if is_from_hibiscus_coast or is_to_hibiscus_coast:
                ...
                logger.info(f"Ã°Å¸Å½Âµ CONCERT PRICING: Hibiscus Coast to Matakana Country Park - flat ${matakana_concert_base}")
            else:
                # From elsewhere - calculate distance to Hibiscus Coast and add $550
                logger.info(f"Ã°Å¸Å½Âµ CONCERT PRICING: {distance_km}km total, charging to Hibiscus Coast + ${matakana_concert_base} concert transfer")
        ...
        # Calculate base price: distance Ãƒâ€” rate for that bracket
        ...
        if is_concert_trip:
            if is_from_hibiscus_coast or is_to_hibiscus_coast:
                ...
                if total_price < matakana_concert_base:
                    logger.info(f"Ã°Å¸Å½Âµ Hibiscus Coast Ã¢â€ â€™ Matakana Country Park: applying flat ${matakana_concert_base}")
                ...
                logger.info(f"Ã°Å¸Å½Âµ Auckland Ã¢â€ â€™ Matakana Country Park: {distance_to_hibiscus}km @ ${rate_to_hibiscus}/km = ${price_to_hibiscus:.2f} + ${matakana_concert_base} = ${total_price:.2f}")
...
async def quick_approve_booking(booking_id: str, action: str = "approve"):
    ...
    try:
        ...
        if not booking:
            return HTMLResponse(content="""
                <html><body style="font-family: Arial; text-align: center; padding: 50px;">
                    <h1 style="color: #DC2626;">Ã¢ÂÅ’ Booking Not Found</h1>
        ...
        if action == "approve":
            ...
            logger.info(f"Ã¢Å“â€¦ Booking {booking_ref} APPROVED via email quick-approve")
            ...
            return HTMLResponse(content=f"""
                <html><body style="font-family: Arial; text-align: center; padding: 50px; background-color: #f0fdf4;">
                    <div style="max-width: 500px; margin: 0 auto; background: white; padding: 40px; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <h1 style="color: #16a34a; font-size: 48px; margin: 0;">Ã¢Å“â€¦</h1>
            ...
            logger.info(f"Ã¢ÂÅ’ Booking {booking_ref} REJECTED via email quick-approve")
            
            return HTMLResponse(content=f"""
                <html><body style="font-family: Arial; text-align: center; padding: 50px; background-color: #fef2f2;">
                    <div style="max-width: 500px; margin: 0 auto; background: white; padding: 40px; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <h1 style="color: #DC2626; font-size: 48px; margin: 0;">Ã¢ÂÅ’</h1>
            ...
            return HTMLResponse(content="""
                <html><body style="font-family: Arial; text-align: center; padding: 50px;">
                    <h1 style="color: #f59e0b;">Ã¢Å¡Â Ã¯Â¸Â Invalid Action</h1>
        ...
        return HTMLResponse(content=f"""
            <html><body style="font-family: Arial; text-align: center; padding: 50px;">
                <h1 style="color: #DC2626;">Ã¢ÂÅ’ Error</h1>
        ...
        if any(key in update_data for key in ['date', 'time', 'returnDate', 'returnTime', 'pickupAddress', 'dropoffAddress']):
            try:
                ...
                if updated_booking and updated_booking.get('calendar_event_id'):
                    ...
                    logger.info(f"Ã°Å¸â€œâ€¦ Calendar event updated for booking {booking_id}")
            except Exception as cal_error:
                logger.warning(f"Ã¢Å¡Â Ã¯Â¸Â Failed to update calendar event: {str(cal_error)}")
    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <div style="padding: 20px; background-color: #ffffff; border: 1px solid #e8e4d9; border-top: none;">
                    <h2 style="color: #333; margin-top: 0;">Ã°Å¸â€œâ€¹ Booking Details</h2>
                    ...
                    <h3 style="color: #333; margin-top: 30px;">Ã°Å¸â€˜Â¤ Customer Information</h3>
                    ...
                    <h3 style="color: #333; margin-top: 30px;">Ã°Å¸Å¡â€” Trip Details</h3>
                    <div style="background-color: #faf8f3; padding: 15px; border-radius: 8px; margin: 15px 0;">
                    ...
                    <h3 style="color: #333; margin-top: 30px;">Ã°Å¸â€™Â° Pricing Details</h3>
                    <div style="background-color: #faf8f3; padding: 15px; border-radius: 8px; margin: 15px 0;">
                        ...
                        {f'<p style="margin: 5px 0; color: #ea580c; font-size: 12px;">Ã¢Å¡Â Ã¯Â¸Â Price was manually overridden</p>' if is_overridden else ''}
                    </div>
                    
                    {'<h3 style="color: #333; margin-top: 30px;">Ã¢Å“Ë†Ã¯Â¸Â Flight Information</h3><div style="background-color: #faf8f3; padding: 15px; border-radius: 8px; margin: 15px 0;"><p style="margin: 5px 0;"><strong>Departure Flight:</strong> ' + booking.get('departureFlightNumber', 'N/A') + ' at ' + booking.get('departureTime', 'N/A') + '</p><p style="margin: 5px 0;"><strong>Arrival Flight:</strong> ' + booking.get('arrivalFlightNumber', 'N/A') + ' at ' + booking.get('arrivalTime', 'N/A') + '</p></div>' if booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber') else ''}
                    
                    {f'<h3 style="color: #333; margin-top: 30px;">Ã°Å¸â€œÂ Special Notes</h3><div style="background-color: #fff8e6; padding: 15px; border-radius: 8px; margin: 15px 0; border-left: 4px solid #D4AF37;"><p style="margin: 0;">{booking.get("notes", "")}</p></div>' if booking.get('notes') else ''}
                    
                    <div style="margin-top: 30px; padding: 15px; background-color: #fff8e6; border-radius: 8px; border-left: 4px solid #D4AF37;">
                        <p style="margin: 0; color: #333;"><strong>Ã°Å¸â€™Â¡ Quick Actions:</strong></p>
        ...
        response = requests.post(
            ...
            data={
                ...
                "to": admin_email,
                "subject": f"Ã°Å¸â€œâ€¹ Booking Details - {booking.get('name', 'Customer')} - {booking.get('id', '')[:8].upper()}",
                "html": html_content
            }
        )
...
EMAIL_TRANSLATIONS = {
    ...
    'zh': {
        'subject': 'Ã©Â¢â€žÃ¨Â®Â¢Ã§Â¡Â®Ã¨Â®Â¤',
        'confirmed': 'Ã©Â¢â€žÃ¨Â®Â¢Ã¥Â·Â²Ã§Â¡Â®Ã¨Â®Â¤Ã¯Â¼Â',
        'greeting': 'Ã¥Â°Å Ã¦â€¢Â¬Ã§Å¡â€ž',
        'intro': 'Ã¦â€šÂ¨Ã§Å¡â€žÃ¨Â¡Å’Ã§Â¨â€¹Ã¥Â·Â²Ã§Â¡Â®Ã¨Â®Â¤Ã£â‚¬â€šÃ¤Â»Â¥Ã¤Â¸â€¹Ã¦ËœÂ¯Ã¦â€šÂ¨Ã§Å¡â€žÃ©Â¢â€žÃ¨Â®Â¢Ã¨Â¯Â¦Ã¦Æ’â€¦Ã¯Â¼Å¡',
        'reference': 'Ã©Â¢â€žÃ¨Â®Â¢Ã§Â¼â€“Ã¥ÂÂ·',
        'service': 'Ã¦Å“ÂÃ¥Å Â¡Ã§Â±Â»Ã¥Å¾â€¹',
        'pickup': 'Ã¤Â¸Å Ã¨Â½Â¦Ã¥Å“Â°Ã§â€šÂ¹',
        'dropoff': 'Ã¤Â¸â€¹Ã¨Â½Â¦Ã¥Å“Â°Ã§â€šÂ¹',
        'date': 'Ã¦â€”Â¥Ã¦Å“Å¸',
        'time': 'Ã¦â€”Â¶Ã©â€”Â´',
        'passengers': 'Ã¤Â¹ËœÃ¥Â®Â¢Ã¤ÂºÂºÃ¦â€¢Â°',
        'total': 'Ã¦â‚¬Â»Ã¨Â´Â¹Ã§â€Â¨',
        'contact_intro': 'Ã¦Ë†â€˜Ã¤Â»Â¬Ã¤Â¼Å¡Ã¥Å“Â¨Ã¦Å½Â¥Ã©â‚¬ÂÃ¦â€”Â¶Ã©â€”Â´Ã¥â€°ÂÃ¨Ââ€Ã§Â³Â»Ã¦â€šÂ¨Ã§Â¡Â®Ã¨Â®Â¤Ã¦â€°â‚¬Ã¦Å“â€°Ã§Â»â€ Ã¨Å â€šÃ£â‚¬â€š',
        'contact': 'Ã¥Â¦â€šÃ¦Å“â€°Ã¤Â»Â»Ã¤Â½â€¢Ã©â€”Â®Ã©Â¢ËœÃ¯Â¼Å’Ã¨Â¯Â·Ã¨Ââ€Ã§Â³Â»Ã¦Ë†â€˜Ã¤Â»Â¬',
        'or_call': 'Ã¦Ë†â€“Ã¨â€¡Â´Ã§â€Âµ',
        'thanks': 'Ã¦â€žÅ¸Ã¨Â°Â¢Ã¦â€šÂ¨Ã©â‚¬â€°Ã¦â€¹Â©BookaRideÃ¯Â¼Â'
    },
    'ja': {
        'subject': 'Ã¤ÂºË†Ã§Â´â€žÃ§Â¢ÂºÃ¨ÂªÂ',
        'confirmed': 'Ã£Ââ€Ã¤ÂºË†Ã§Â´â€žÃ£ÂÅ’Ã§Â¢ÂºÃ¥Â®Å¡Ã£Ââ€”Ã£ÂÂ¾Ã£Ââ€”Ã£ÂÅ¸Ã¯Â¼Â',
        'greeting': 'Ã¦Â§Ëœ',
        'intro': 'Ã£Ââ€Ã¤ÂºË†Ã§Â´â€žÃ£ÂÅ’Ã§Â¢ÂºÃ¥Â®Å¡Ã£Ââ€”Ã£ÂÂ¾Ã£Ââ€”Ã£ÂÅ¸Ã£â‚¬â€šÃ¤ÂºË†Ã§Â´â€žÃ¨Â©Â³Ã§Â´Â°Ã£ÂÂ¯Ã¤Â»Â¥Ã¤Â¸â€¹Ã£ÂÂ®Ã©â‚¬Å¡Ã£â€šÅ Ã£ÂÂ§Ã£Ââ„¢Ã¯Â¼Å¡',
        'reference': 'Ã¤ÂºË†Ã§Â´â€žÃ§â€¢ÂªÃ¥ÂÂ·',
        'service': 'Ã£â€šÂµÃ£Æ’Â¼Ã£Æ’â€œÃ£â€šÂ¹Ã£â€šÂ¿Ã£â€šÂ¤Ã£Æ’â€”',
        'pickup': 'Ã¤Â¹â€”Ã¨Â»Å Ã¥Â Â´Ã¦â€°â‚¬',
        'dropoff': 'Ã©â„¢ÂÃ¨Â»Å Ã¥Â Â´Ã¦â€°â‚¬',
        'date': 'Ã¦â€”Â¥Ã¤Â»Ëœ',
        'time': 'Ã¦â„¢â€šÃ©â€“â€œ',
        'passengers': 'Ã¤Â¹â€”Ã¥Â®Â¢Ã¦â€¢Â°',
        'total': 'Ã¥ÂË†Ã¨Â¨Ë†Ã©â€¡â€˜Ã©Â¡Â',
        'contact_intro': 'Ã¤Â¹â€”Ã¨Â»Å Ã¦â„¢â€šÃ©â€“â€œÃ£ÂÅ’Ã¨Â¿â€˜Ã£ÂÂ¥Ã£ÂÂÃ£ÂÂ¾Ã£Ââ€”Ã£ÂÅ¸Ã£â€šâ€°Ã£â‚¬ÂÃ¨Â©Â³Ã§Â´Â°Ã§Â¢ÂºÃ¨ÂªÂÃ£ÂÂ®Ã£ÂÅ¸Ã£â€šÂÃ£Ââ€Ã©â‚¬Â£Ã§ÂµÂ¡Ã£Ââ€žÃ£ÂÅ¸Ã£Ââ€”Ã£ÂÂ¾Ã£Ââ„¢Ã£â‚¬â€š',
        'contact': 'Ã£Ââ€Ã¨Â³ÂªÃ¥â€¢ÂÃ£ÂÅ’Ã£Ââ€Ã£Ââ€“Ã£Ââ€žÃ£ÂÂ¾Ã£Ââ€”Ã£ÂÅ¸Ã£â€šâ€°Ã£â‚¬ÂÃ£ÂÅ Ã¥â€¢ÂÃ£Ââ€žÃ¥ÂË†Ã£â€šÂÃ£Ââ€ºÃ£ÂÂÃ£ÂÂ Ã£Ââ€¢Ã£Ââ€ž',
        'or_call': 'Ã£ÂÂ¾Ã£ÂÅ¸Ã£ÂÂ¯Ã£ÂÅ Ã©â€ºÂ»Ã¨Â©Â±Ã£ÂÂÃ£ÂÂ Ã£Ââ€¢Ã£Ââ€ž',
        'thanks': 'BookaRideÃ£â€šâ€™Ã£Ââ€Ã¥Ë†Â©Ã§â€Â¨Ã£Ââ€žÃ£ÂÅ¸Ã£ÂÂ Ã£ÂÂÃ£Ââ€šÃ£â€šÅ Ã£ÂÅ’Ã£ÂÂ¨Ã£Ââ€ Ã£Ââ€Ã£Ââ€“Ã£Ââ€žÃ£ÂÂ¾Ã£Ââ„¢Ã¯Â¼Â'
    },
    'ko': {
        'subject': 'Ã¬ËœË†Ã¬â€¢Â½ Ã­â„¢â€¢Ã¬ÂÂ¸',
        'confirmed': 'Ã¬ËœË†Ã¬â€¢Â½Ã¬ÂÂ´ Ã­â„¢â€¢Ã¬Â â€¢Ã«ÂËœÃ¬â€”Ë†Ã¬Å ÂµÃ«â€¹Ë†Ã«â€¹Â¤!',
        'greeting': 'ÃªÂ³Â ÃªÂ°ÂÃ«â€¹ËœÃªÂ»Ëœ',
        'intro': 'Ã¬ËœË†Ã¬â€¢Â½Ã¬ÂÂ´ Ã­â„¢â€¢Ã¬Â â€¢Ã«ÂËœÃ¬â€”Ë†Ã¬Å ÂµÃ«â€¹Ë†Ã«â€¹Â¤. Ã¬ËœË†Ã¬â€¢Â½ Ã¬Æ’ÂÃ¬â€žÂ¸ Ã¬Â â€¢Ã«Â³Â´Ã«Å â€ Ã«â€¹Â¤Ã¬ÂÅ’ÃªÂ³Â¼ ÃªÂ°â„¢Ã¬Å ÂµÃ«â€¹Ë†Ã«â€¹Â¤:',
        'reference': 'Ã¬ËœË†Ã¬â€¢Â½ Ã«Â²Ë†Ã­ËœÂ¸',
        'service': 'Ã¬â€žÅ“Ã«Â¹â€žÃ¬Å Â¤ Ã¬Å“Â Ã­Ëœâ€¢',
        'pickup': 'Ã­Æ’â€˜Ã¬Å Â¹ Ã¬Å“â€žÃ¬Â¹Ëœ',
        'dropoff': 'Ã­â€¢ËœÃ¬Â°Â¨ Ã¬Å“â€žÃ¬Â¹Ëœ',
        'date': 'Ã«â€šÂ Ã¬Â§Å“',
        'time': 'Ã¬â€¹Å“ÃªÂ°â€ž',
        'passengers': 'Ã¬Å Â¹ÃªÂ°Â Ã¬Ë†Ëœ',
        'total': 'Ã¬Â´Â ÃªÂ²Â°Ã¬Â Å“ ÃªÂ¸Ë†Ã¬â€¢Â¡',
        'contact_intro': 'Ã­Æ’â€˜Ã¬Å Â¹ Ã¬â€¹Å“ÃªÂ°â€žÃ¬ÂÂ´ ÃªÂ°â‚¬ÃªÂ¹Å’Ã¬â€ºÅ’Ã¬Â§â‚¬Ã«Â©Â´ Ã«ÂªÂ¨Ã«â€œÂ  Ã¬â€žÂ¸Ã«Â¶â‚¬ Ã¬â€šÂ¬Ã­â€¢Â­Ã¬Ââ€ž Ã­â„¢â€¢Ã¬ÂÂ¸Ã­â€¢ËœÃªÂ¸Â° Ã¬Å“â€žÃ­â€¢Â´ Ã¬â€”Â°Ã«ÂÂ½Ã«â€œÅ“Ã«Â¦Â¬ÃªÂ²Â Ã¬Å ÂµÃ«â€¹Ë†Ã«â€¹Â¤.',
        'contact': 'Ã«Â¬Â¸Ã¬ÂËœÃ¬â€šÂ¬Ã­â€¢Â­Ã¬ÂÂ´ Ã¬Å¾Ë†Ã¬Å“Â¼Ã¬â€¹Å“Ã«Â©Â´ Ã¬â€”Â°Ã«ÂÂ½Ã¬Â£Â¼Ã¬â€žÂ¸Ã¬Å¡â€',
        'or_call': 'Ã«ËœÂÃ«Å â€ Ã¬Â â€žÃ­â„¢â€Ã¬Â£Â¼Ã¬â€žÂ¸Ã¬Å¡â€',
        'thanks': 'BookaRideÃ«Â¥Â¼ Ã¬ÂÂ´Ã¬Å¡Â©Ã­â€¢Â´ Ã¬Â£Â¼Ã¬â€¦â€Ã¬â€žÅ“ ÃªÂ°ÂÃ¬â€šÂ¬Ã­â€¢Â©Ã«â€¹Ë†Ã«â€¹Â¤!'
    },
    'fr': {
        'subject': 'Confirmation de RÃƒÂ©servation',
        'confirmed': 'RÃƒÂ©servation ConfirmÃƒÂ©e!',
        'greeting': 'Cher',
        'intro': 'Votre trajet a ÃƒÂ©tÃƒÂ© confirmÃƒÂ©. Voici les dÃƒÂ©tails de votre rÃƒÂ©servation:',
        'reference': 'RÃƒÂ©fÃƒÂ©rence de RÃƒÂ©servation',
        'service': 'Type de Service',
        'pickup': 'Lieu de Prise en Charge',
        'dropoff': 'Lieu de DÃƒÂ©pose',
        'date': 'Date',
        'time': 'Heure',
        'passengers': 'Passagers',
        'total': 'Total PayÃƒÂ©',
        'contact_intro': 'Nous vous contacterons avant votre heure de prise en charge pour confirmer tous les dÃƒÂ©tails.',
        'contact': 'Pour toute question, veuillez nous contacter ÃƒÂ ',
        'or_call': 'ou appelez',
        'thanks': 'Merci d\'avoir choisi BookaRide!'
    },
    'hi': {
        'subject': 'Ã Â¤Â¬Ã Â¥ÂÃ Â¤â€¢Ã Â¤Â¿Ã Â¤â€šÃ Â¤â€” Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â·Ã Â¥ÂÃ Â¤Å¸Ã Â¤Â¿',
        'confirmed': 'Ã Â¤Â¬Ã Â¥ÂÃ Â¤â€¢Ã Â¤Â¿Ã Â¤â€šÃ Â¤â€” Ã Â¤â€¢Ã Â¥â‚¬ Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â·Ã Â¥ÂÃ Â¤Å¸Ã Â¤Â¿ Ã Â¤Â¹Ã Â¥â€¹ Ã Â¤â€”Ã Â¤Ë†!',
        'greeting': 'Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â°Ã Â¤Â¿Ã Â¤Â¯',
        'intro': 'Ã Â¤â€ Ã Â¤ÂªÃ Â¤â€¢Ã Â¥â‚¬ Ã Â¤Â°Ã Â¤Â¾Ã Â¤â€¡Ã Â¤Â¡ Ã Â¤â€¢Ã Â¥â‚¬ Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â·Ã Â¥ÂÃ Â¤Å¸Ã Â¤Â¿ Ã Â¤Â¹Ã Â¥â€¹ Ã Â¤â€”Ã Â¤Ë† Ã Â¤Â¹Ã Â¥Ë†Ã Â¥Â¤ Ã Â¤Â¯Ã Â¤Â¹Ã Â¤Â¾Ã Â¤â€š Ã Â¤â€ Ã Â¤ÂªÃ Â¤â€¢Ã Â¥â‚¬ Ã Â¤Â¬Ã Â¥ÂÃ Â¤â€¢Ã Â¤Â¿Ã Â¤â€šÃ Â¤â€” Ã Â¤ÂµÃ Â¤Â¿Ã Â¤ÂµÃ Â¤Â°Ã Â¤Â£ Ã Â¤Â¹Ã Â¥Ë†Ã Â¤â€š:',
        'reference': 'Ã Â¤Â¬Ã Â¥ÂÃ Â¤â€¢Ã Â¤Â¿Ã Â¤â€šÃ Â¤â€” Ã Â¤Â¸Ã Â¤â€šÃ Â¤Â¦Ã Â¤Â°Ã Â¥ÂÃ Â¤Â­',
        'service': 'Ã Â¤Â¸Ã Â¥â€¡Ã Â¤ÂµÃ Â¤Â¾ Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â°Ã Â¤â€¢Ã Â¤Â¾Ã Â¤Â°',
        'pickup': 'Ã Â¤ÂªÃ Â¤Â¿Ã Â¤â€¢Ã Â¤â€¦Ã Â¤Âª',
        'dropoff': 'Ã Â¤Â¡Ã Â¥ÂÃ Â¤Â°Ã Â¥â€°Ã Â¤Âª-Ã Â¤â€˜Ã Â¤Â«',
        'date': 'Ã Â¤Â¤Ã Â¤Â¾Ã Â¤Â°Ã Â¥â‚¬Ã Â¤â€“',
        'time': 'Ã Â¤Â¸Ã Â¤Â®Ã Â¤Â¯',
        'passengers': 'Ã Â¤Â¯Ã Â¤Â¾Ã Â¤Â¤Ã Â¥ÂÃ Â¤Â°Ã Â¥â‚¬',
        'total': 'Ã Â¤â€¢Ã Â¥ÂÃ Â¤Â² Ã Â¤Â­Ã Â¥ÂÃ Â¤â€”Ã Â¤Â¤Ã Â¤Â¾Ã Â¤Â¨',
        'contact_intro': 'Ã Â¤Â¹Ã Â¤Â® Ã Â¤â€ Ã Â¤ÂªÃ Â¤â€¢Ã Â¥â€¡ Ã Â¤ÂªÃ Â¤Â¿Ã Â¤â€¢Ã Â¤â€¦Ã Â¤Âª Ã Â¤Â¸Ã Â¤Â®Ã Â¤Â¯ Ã Â¤Â¸Ã Â¥â€¡ Ã Â¤ÂªÃ Â¤Â¹Ã Â¤Â²Ã Â¥â€¡ Ã Â¤Â¸Ã Â¤Â­Ã Â¥â‚¬ Ã Â¤ÂµÃ Â¤Â¿Ã Â¤ÂµÃ Â¤Â°Ã Â¤Â£Ã Â¥â€¹Ã Â¤â€š Ã Â¤â€¢Ã Â¥â‚¬ Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â·Ã Â¥ÂÃ Â¤Å¸Ã Â¤Â¿ Ã Â¤â€¢Ã Â¥â€¡ Ã Â¤Â²Ã Â¤Â¿Ã Â¤Â Ã Â¤Â¸Ã Â¤â€šÃ Â¤ÂªÃ Â¤Â°Ã Â¥ÂÃ Â¤â€¢ Ã Â¤â€¢Ã Â¤Â°Ã Â¥â€¡Ã Â¤â€šÃ Â¤â€”Ã Â¥â€¡Ã Â¥Â¤',
        'contact': 'Ã Â¤Â¯Ã Â¤Â¦Ã Â¤Â¿ Ã Â¤â€ Ã Â¤ÂªÃ Â¤â€¢Ã Â¥â€¡ Ã Â¤â€¢Ã Â¥â€¹Ã Â¤Ë† Ã Â¤ÂªÃ Â¥ÂÃ Â¤Â°Ã Â¤Â¶Ã Â¥ÂÃ Â¤Â¨ Ã Â¤Â¹Ã Â¥Ë†Ã Â¤â€š, Ã Â¤Â¤Ã Â¥â€¹ Ã Â¤â€¢Ã Â¥Æ’Ã Â¤ÂªÃ Â¤Â¯Ã Â¤Â¾ Ã Â¤Â¹Ã Â¤Â®Ã Â¤Â¸Ã Â¥â€¡ Ã Â¤Â¸Ã Â¤â€šÃ Â¤ÂªÃ Â¤Â°Ã Â¥ÂÃ Â¤â€¢ Ã Â¤â€¢Ã Â¤Â°Ã Â¥â€¡Ã Â¤â€š',
        'or_call': 'Ã Â¤Â¯Ã Â¤Â¾ Ã Â¤â€¢Ã Â¥â€°Ã Â¤Â² Ã Â¤â€¢Ã Â¤Â°Ã Â¥â€¡Ã Â¤â€š',
        'thanks': 'BookaRide Ã Â¤Å¡Ã Â¥ÂÃ Â¤Â¨Ã Â¤Â¨Ã Â¥â€¡ Ã Â¤â€¢Ã Â¥â€¡ Ã Â¤Â²Ã Â¤Â¿Ã Â¤Â Ã Â¤Â§Ã Â¤Â¨Ã Â¥ÂÃ Â¤Â¯Ã Â¤ÂµÃ Â¤Â¾Ã Â¤Â¦!'
    }
}
...
def send_booking_confirmation_sms(booking: dict):
    ...
    try:
        ...
        if booking.get('bookReturn') or booking.get('returnDate'):
            ...
            return_text = f"\n\nÃ°Å¸â€â€ž Return: {return_date} at {return_time}"
            if return_flight:
...
Thank you for booking with us!"""
        ...
        logger.info(f"Ã°Å¸â€œÂ± Sending confirmation SMS to: {formatted_phone}")
        ...
        logger.info(f"Ã¢Å“â€¦ Confirmation SMS sent to {formatted_phone} - SID: {message.sid}")
        return True
...
def send_customer_confirmation(booking: dict):
    ...
    if booking_id:
        try:
            ...
            sync_client.close()
            logger.info(f"Ã¢Å“â€¦ Confirmation status updated for booking {booking_id}")
        except Exception as e:
...
def send_reminder_email(booking: dict):
    ...
    try:
        ...
        if booking_status in ['cancelled', 'canceled', 'deleted']:
            logger.warning(f"Ã¢Å¡Â Ã¯Â¸Â Skipping reminder email for CANCELLED booking: {booking.get('name')} (Ref: {booking.get('referenceNumber')})")
            return False
            ...
            airport_instructions = f'''
                    <div style="background: linear-gradient(135deg, #1a365d 0%, #2c5282 100%); border-radius: 12px; padding: 25px; margin: 25px 0; color: white;">
                        <h3 style="margin: 0 0 15px 0; font-size: 18px; display: flex; align-items: center;">
                            Ã¢Å“Ë†Ã¯Â¸Â INTERNATIONAL ARRIVALS - MEETING POINT
                        </h3>
            ...
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
                ...
                <div style="padding: 25px;">
                    ...
                    <div style="background: #faf8f3; border-radius: 10px; padding: 20px; margin: 20px 0; border: 1px solid #e8e4d9;">
                        <h3 style="margin: 0 0 15px 0; color: #1a1a1a; font-size: 16px;">Ã°Å¸â€œâ€¹ BOOKING DETAILS</h3>
...
def send_reminder_sms(booking: dict):
    ...
    try:
        ...
        booking_status = booking.get('status', '').lower()
        if booking_status in ['cancelled', 'canceled', 'deleted']:
            logger.warning(f"Ã¢Å¡Â Ã¯Â¸Â Skipping reminder SMS for CANCELLED booking: {booking.get('name')} (Ref: {booking.get('referenceNumber')})")
            return False
...
3. Walk to Allpress Espresso CafÃƒÂ©
4. All drivers wait in front of the cafÃƒÂ©
...
Questions? +64 21 743 321"""
        ...
        logger.info(f"Ã°Å¸â€œÂ± Sending reminder SMS to: {formatted_phone}")
        ...
        logger.info(f"Ã¢Å“â€¦ Reminder SMS sent to {formatted_phone} - SID: {message.sid}")
        return True
...
async def track_flight(flight_number: str):
    ...
    try:
        ...
        logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â Flight {fn}: {status} - Arriving {arr_code} at {format_time(arr_actual)}")
...
async def handle_incoming_email(request: Request):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ§ Incoming email from: {reply_to_email}, Subject: {subject}")
...
Write a professional, helpful response:"""
        ...
        if response.status_code == 200:
            logger.info(f"Ã¢Å“â€¦ AI auto-reply sent to {reply_to_email}")
    ...
    try:
        ...
        for booking in abandoned:
            try:
                ...
                subject = "Complete Your BookaRide Booking Ã°Å¸Å¡â€”"
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="background: #f5f5f5; padding: 20px; text-align: center; font-size: 12px; color: #666;">
                        <p>Ã‚Â© BookaRide NZ | Auckland Airport Transfers</p>
                    </div>
                </div>
                """
...
- Use emojis sparingly but naturally Ã°Å¸Å¡â€”
...
Respond helpfully and naturally as the BookaRide assistant:"""
        ...
        # Fallback response
        return {
            "response": "I apologize, I'm having a brief technical issue. For immediate assistance, please call us at 0800 BOOK A RIDE or visit bookaride.co.nz/book-now to make a booking. We're here to help! Ã°Å¸Å¡â€”"
        }
...
async def send_daily_reminders_core(source: str = "unknown"):
    ...
    if reminder_lock.locked():
        logger.info(f"Ã°Å¸â€â€™ [{source}] Reminder job already running, skipping to prevent duplicates")
    ...
    async with reminder_lock:
        try:
            ...
            logger.info(f"Ã°Å¸â€â€ [{source}] Checking reminders - NZ time: {nz_now.strftime('%Y-%m-%d %H:%M:%S')}, Tomorrow: {nz_tomorrow}")
            ...
            logger.info(f"Ã°Å¸â€â€ [{source}] Found {len(bookings)} bookings needing reminders for {nz_tomorrow}")
            ...
            for booking in bookings:
                ...
                if update_result.modified_count == 0:
                    skipped_count += 1
                    logger.debug(f"Ã¢ÂÂ­Ã¯Â¸Â Skipping {booking_name} - already being processed by another job")
                    continue
                ...
                try:
                    ...
                    if booking.get('email'):
                        ...
                        if email_sent:
                            logger.info(f"Ã¢Å“â€°Ã¯Â¸Â Reminder email sent to {booking.get('email')}")
                    
                    # Send SMS reminder
                    if booking.get('phone'):
                        sms_sent = send_reminder_sms(booking)
                        if sms_sent:
                            logger.info(f"Ã°Å¸â€œÂ± Reminder SMS sent to {booking.get('phone')}")
                    ...
                    logger.error(f"Ã¢ÂÅ’ Error sending reminder to {booking_name}: {str(send_error)}")
            ...
            logger.info(f"Ã¢Å“â€¦ [{source}] Reminders complete: {sent_count} sent, {skipped_count} skipped")
            return {"success": True, "reminders_sent": sent_count, "skipped": skipped_count, "source": source}
            
        except Exception as e:
            logger.error(f"Ã¢ÂÅ’ [{source}] Reminder error: {str(e)}")
            raise
...
async def send_booking_notification_to_admin(booking: dict):
    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <div style="background: linear-gradient(135deg, #D4AF37 0%, #B8960C 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
                    <h1 style="margin: 0;">BookaRide.co.nz</h1>
                    <p style="margin: 5px 0; font-size: 14px; color: rgba(255,255,255,0.9);">Ã°Å¸â€â€ New Booking Received</p>
                </div>
                ...
                <div style="padding: 20px; background-color: #ffffff; border: 1px solid #e8e4d9; border-top: none;">
                    ...
                    <div style="background-color: #faf8f3; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #D4AF37;">
                        ...
                        <p style="margin: 5px 0;"><strong>Pickup:</strong> {booking.get('pickupAddress', 'N/A')}</p>
                        <p style="margin: 5px 0;"><strong>Drop-off:</strong> {booking.get('dropoffAddress', 'N/A')}</p>
                        {'<hr style="border: 0; border-top: 1px solid #e8e4d9; margin: 15px 0;"><div style="background-color: #e3f2fd; padding: 10px; border-radius: 5px; border-left: 4px solid #2196F3;"><p style="margin: 0; font-weight: bold; color: #1565C0;">Ã¢Å“Ë†Ã¯Â¸Â FLIGHT INFORMATION</p><p style="margin: 5px 0 0 0;"><strong>Flight Number:</strong> ' + (booking.get('flightNumber') or booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber') or 'N/A') + '</p></div>' if (booking.get('flightNumber') or booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber')) else ''}
                        {'<hr style="border: 0; border-top: 1px solid #e8e4d9; margin: 15px 0;"><div style="background-color: #f3e5f5; padding: 10px; border-radius: 5px; border-left: 4px solid #9C27B0;"><p style="margin: 0; font-weight: bold; color: #7B1FA2;">Ã°Å¸â€â€ž RETURN TRIP BOOKED</p><p style="margin: 5px 0 0 0;"><strong>Return Date:</strong> ' + format_date_ddmmyyyy(booking.get('returnDate', '')) + ' at ' + format_time_ampm(booking.get('returnTime', '')) + '</p><p style="margin: 5px 0 0 0;"><strong>Return Flight:</strong> ' + (booking.get('returnFlightNumber') or booking.get('returnDepartureFlightNumber') or 'NOT PROVIDED - FOLLOW UP REQUIRED') + '</p></div>' if booking.get('bookReturn') or booking.get('returnDate') else ''}
                        <hr style="border: 0; border-top: 2px solid #D4AF37; margin: 15px 0;">
                    ...
                    <div style="margin-top: 20px; padding: 15px; background-color: #fff8e6; border-radius: 8px; border-left: 4px solid #D4AF37;">
                        <p style="margin: 0; font-weight: bold; color: #333;">Ã¢Å¡Â¡ Action Required:</p>
        ...
        response = requests.post(
            ...
            data={
                "from": f"BookaRide System <{sender_email}>",
                "to": admin_email,
                "subject": f"Ã°Å¸â€â€ New Booking - {booking.get('name', 'Customer')} - {formatted_date} - Ref: {booking_ref}",
                "html": html_content
            }
        )
    ...
    try:
        ...
        if twilio_sid and twilio_token and twilio_from:
            ...
            sms_body = f"Ã°Å¸â€â€ NEW BookaRide: {booking.get('name')} booked for {formatted_date} {formatted_time}. Ref: #{booking_ref}. Check admin dashboard."
            ...
            logger.info(f"Ã¢Å“â€¦ Admin SMS sent for new booking: #{booking_ref}")
            sms_sent = True
        else:
...
async def send_urgent_approval_notification(booking: dict):
    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <div style="background: linear-gradient(135deg, #DC2626 0%, #B91C1C 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
                    <h1 style="margin: 0;">Ã°Å¸Å¡Â¨ URGENT APPROVAL REQUIRED</h1>
                ...
                <div style="padding: 20px; background-color: #ffffff; border: 1px solid #fca5a5; border-top: none;">
                    <div style="background-color: #fef2f2; padding: 15px; border-radius: 8px; border-left: 4px solid #DC2626; margin-bottom: 20px;">
                        <p style="margin: 0; font-size: 18px; font-weight: bold; color: #991B1B;">Ã¢Å¡Â Ã¯Â¸Â This booking requires your manual approval</p>
                    ...
                    <div style="background-color: #fff7ed; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f97316;">
                        <h3 style="margin-top: 0; color: #c2410c;">Ã°Å¸â€œâ€¦ Pickup Details</h3>
                    ...
                    <div style="margin-top: 20px; padding: 20px; background-color: #fef2f2; border-radius: 8px; border: 2px solid #DC2626; text-align: center;">
                        <p style="margin: 0; font-weight: bold; font-size: 16px; color: #991B1B;">Ã°Å¸â€â€ ACTION REQUIRED</p>
                        <p style="margin: 10px 0 0 0; font-size: 14px; color: #7F1D1D;">Approve or reject this booking:</p>
                        
                        <div style="margin-top: 20px;">
                            <a href="https://bookaride.co.nz/api/booking/quick-approve/{booking.get('id')}?action=approve" style="display: inline-block; margin: 5px; padding: 15px 40px; background-color: #16a34a; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">Ã¢Å“â€¦ APPROVE</a>
                            <a href="https://bookaride.co.nz/api/booking/quick-approve/{booking.get('id')}?action=reject" style="display: inline-block; margin: 5px; padding: 15px 40px; background-color: #DC2626; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">Ã¢ÂÅ’ REJECT</a>
                        </div>
        ...
        response = requests.post(
            ...
            data={
                ...
                "to": admin_email,
                "subject": f"Ã°Å¸Å¡Â¨ URGENT APPROVAL - {booking.get('name', 'Customer')} - {formatted_date} {booking.get('time', '')} - Ref: {booking_ref}",
                "html": html_content
            }
        )
    ...
    try:
        ...
        if twilio_sid and twilio_token and twilio_from:
            ...
            sms_body = f"""Ã°Å¸Å¡Â¨ URGENT BOOKING #{booking_ref}
...
Ã¢Å¡Â Ã¯Â¸Â Reply YES to APPROVE or NO to DECLINE"""
            ...
            logger.info(f"Ã¢Å“â€¦ Urgent SMS sent to admin {admin_phone} for booking: #{booking_ref} - SID: {message.sid}")
            ...
            try:
                ...
                sync_client.close()
                logger.info(f"Ã¢Å“â€¦ Stored pending approval for booking #{booking_ref}")
...
async def send_driver_notification(booking: dict, driver: dict, trip_type: str = "OUTBOUND"):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ§ send_driver_notification called for {trip_type} trip")
        logger.info(f"Ã°Å¸â€œÂ§ Booking data: name={booking.get('name')}, phone={booking.get('phone')}, pickup={booking.get('pickupAddress')}, date={booking.get('date')}, time={booking.get('time')}")
        logger.info(f"Ã°Å¸â€œÂ§ Driver data: name={driver.get('name')}, email={driver.get('email')}")
        ...
        logger.info(f"Ã°Å¸â€œÂ§ Formatted: date={formatted_date}, time={formatted_time}, ref={booking_ref}")
        ...
        if booking.get('driver_payout_override') is not None:
            ...
            logger.info(f"Ã°Å¸â€œÂ§ Using manual driver payout override: ${driver_payout:.2f}")
        else:
            ...
            if has_return:
                ...
                logger.info(f"Ã°Å¸â€œÂ§ Return booking: {trip_type} trip price = ${trip_price:.2f} (subtotal ${subtotal:.2f})")
            else:
            ...
            logger.info(f"Ã°Å¸â€œÂ§ Driver gets full subtotal (customer pays Stripe fee separately)")
            
            # Round to 2 decimal places
            driver_payout = round(driver_payout, 2)
        
        logger.info(f"Ã°Å¸â€œÂ§ Driver Payout: ${driver_payout:.2f} for {trip_type} trip")
            ...
            if booking.get('bookReturn'):
                ...
                if return_date:
                    ...
                    <div style="background-color: #f3e5f5; padding: 15px; border-radius: 8px; margin: 15px 0; border-left: 4px solid #9C27B0;">
                        <p style="margin: 0; font-weight: bold; color: #7B1FA2;">Ã°Å¸â€â€ž RETURN TRIP DETAILS</p>
                        ...
                        {'<p style="margin: 5px 0;"><strong>Ã¢Å“Ë†Ã¯Â¸Â Return Flight:</strong> ' + return_flight + '</p>' if return_flight else '<p style="margin: 5px 0; color: #d32f2f;"><strong>Ã¢Å¡Â Ã¯Â¸Â Return Flight:</strong> NOT PROVIDED</p>'}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto;">
        ...
        <tr>
            <td style="padding: 20px; background-color: #f5f5f5;">
                <h2 style="color: #1a1a1a; margin-top: 0;">Ã¢Å“â€¦ New Booking Assignment - {trip_type} TRIP</h2>
                ...
                <table width="100%" cellpadding="15" cellspacing="0" style="background-color: white; border-radius: 8px; margin: 20px 0; border-left: 4px solid #D4AF37;">
                    <tr>
                        <td>
                            ...
                            <p style="margin: 5px 0;"><strong>Passengers:</strong> {booking.get('passengers', 'N/A')}</p>
                            {'<div style="background-color: #e3f2fd; padding: 10px; border-radius: 5px; margin: 10px 0; border-left: 4px solid #2196F3;"><p style="margin: 0; font-weight: bold; color: #1565C0;">Ã¢Å“Ë†Ã¯Â¸Â FLIGHT: ' + (booking.get('flightNumber') or booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber') or 'N/A') + '</p></div>' if (booking.get('flightNumber') or booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber')) else ''}
                            <hr style="border: 0; border-top: 2px solid #D4AF37; margin: 15px 0;">
                            <p style="margin: 5px 0; font-size: 18px;"><strong>Ã°Å¸â€™Â° Your Payout: ${driver_payout:.2f} NZD</strong></p>
                        </td>
                    </tr>
                </table>
                {return_html}
...
"""
            ...
            if response.status_code == 200:
                logger.info(f"Ã¢Å“â€¦ Driver notification email sent to {driver.get('email')}")
                logger.info(f"Ã°Å¸â€œÂ§ Mailgun response: {response.text}")
            else:
                logger.error(f"Ã¢ÂÅ’ Failed to send driver email: {response.status_code} - {response.text}")
        ...
        try:
            ...
            if account_sid and auth_token and twilio_phone:
                ...
                if driver_phone:
                    logger.info(f"Ã°Å¸â€œÂ± Sending driver SMS to: {driver_phone} (original: {driver.get('phone')})")
                    ...
                    payment_display = {
                        'paid': 'Ã¢Å“â€¦ PAID',
                        'cash': 'Ã°Å¸â€™Âµ CASH',
                        'pay-on-pickup': 'Ã°Å¸Å¡â€” PAY ON PICKUP',
                        'xero-invoiced': 'Ã°Å¸â€œâ€ž INVOICED',
                        'unpaid': 'Ã¢Å¡Â Ã¯Â¸Â UNPAID'
                    ...
                    flight_text = f"\nÃ¢Å“Ë†Ã¯Â¸Â Flight: {flight_num}" if flight_num else ""
...
Ã¢Å¡Â Ã¯Â¸Â REPLY YES to confirm you received this job.
...
Check your email for full details."""
                    ...
                    logger.info(f"Ã¢Å“â€¦ Driver notification SMS sent to {driver_phone} - SID: {message.sid}")
                else:
                    logger.warning(f"Ã¢Å¡Â Ã¯Â¸Â Driver {driver.get('name')} has no phone number - SMS not sent")
            else:
                logger.warning("Ã¢Å¡Â Ã¯Â¸Â Twilio credentials not configured for driver SMS")
        except Exception as sms_error:
            logger.error(f"Ã¢ÂÅ’ Error sending driver SMS: {str(sms_error)}")
    ...
    try:
        ...
        if service_account_json:
            ...
            try:
                ...
                logger.info("Ã¢Å“â€¦ Service account credentials loaded from environment variable")
                return creds
        ...
        if service_account_file and os.path.exists(service_account_file):
            ...
            logger.info("Ã¢Å“â€¦ Service account credentials loaded from file")
            return creds
...
async def create_calendar_event(booking: dict):
    ...
    try:
        ...
        outbound_event = {
            'summary': f"{customer_name} Ã¢â€ â€™ {eng['dropoff'].split(',')[0]}" + (" + Return" if has_return else ""),
...
Ã¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€ÂÃ¢â€Â'''}
        ...
        if has_return and booking.get('returnDate') and booking.get('returnTime'):
            ...
            if return_nz_dt:
                ...
                return_event = {
                    'summary': f"{customer_name} Ã¢â€ Â Return from {eng['dropoff'].split(',')[0]}",
                                ...
                                if mailgun_key:
                                    requests.post(
                                        ...
                                        data={
                                            ...
                                            "subject": f"Ã°Å¸Å¡Â Shuttle Booking Confirmed - {shuttle_booking['date']} {shuttle_booking['departureTime']}",
                                            "html": f"""
                                            ...
                                            <h3>Payment Info:</h3>
                                            <p>Ã°Å¸â€™Â³ A hold of <strong>${shuttle_booking.get('totalEstimated', 100)}</strong> has been placed on your card.</p>
                                            <p>Ã¢Å“â€¦ You will only be charged the <strong>final price</strong> when the shuttle arrives at the airport.</p>
                                            <p>Ã°Å¸â€™Â° The more passengers on your shuttle, the cheaper everyone pays!</p>
...
async def twilio_sms_webhook(request: Request):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ± Incoming SMS from {from_number}: {message_body}")
        ...
        if is_admin and message_body in ['YES', 'Y', 'YEP', 'CONFIRM', 'APPROVE', 'OK', 'OKAY', 'ACCEPTED']:
            ...
            if pending:
                ...
                logger.info(f"Ã¢Å“â€¦ Admin approved booking #{booking_ref} ({customer_name}) via SMS")
                ...
                try:
                    ...
                    if twilio_sid and twilio_token:
                        client = Client(twilio_sid, twilio_token)
                        client.messages.create(
                            body=f"Ã¢Å“â€¦ Booking #{booking_ref} for {customer_name} APPROVED! Customer confirmation sent.",
                            from_=twilio_from,
                            to=admin_phone
                        )
                except Exception as e:
            ...
            if pending:
                ...
                logger.info(f"Ã¢ÂÅ’ Admin declined booking #{booking_ref} ({customer_name}) via SMS")
                ...
                try:
                    ...
                    if twilio_sid and twilio_token:
                        client = Client(twilio_sid, twilio_token)
                        client.messages.create(
                            body=f"Ã¢ÂÅ’ Booking #{booking_ref} for {customer_name} DECLINED. Customer notified.",
                            from_=twilio_from,
                            to=admin_phone
                        )
                except Exception as e:
        ...
        if message_body in ['YES', 'Y', 'YEP', 'CONFIRM', 'CONFIRMED', 'OK', 'OKAY', 'ACCEPTED']:
            ...
            if driver:
                ...
                if booking:
                    ...
                    logger.info(f"Ã¢Å“â€¦ Driver {driver_name} acknowledged {trip_type} trip for booking #{booking_ref} (+ {batch_result.modified_count} other bookings)")
                    ...
                    try:
                        ...
                        if account_sid and auth_token and twilio_phone:
                            client = Client(account_sid, auth_token)
                            client.messages.create(
                                body=f"Ã¢Å“â€¦ Thanks {driver_name}! Job #{booking_ref} confirmed. Customer: {booking.get('name')} on {format_date_ddmmyyyy(booking.get('date'))} at {booking.get('time')}",
                                from_=twilio_phone,
                                to=from_number
                            )
                    except Exception as sms_error:
                        ...
                        if mailgun_api_key and mailgun_domain:
                            ...
                            <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
                                <div style="background: #22c55e; color: white; padding: 15px; border-radius: 8px 8px 0 0; text-align: center;">
                                    <h2 style="margin: 0;">Ã¢Å“â€¦ Driver Acknowledged Job</h2>
                                </div>
                            ...
                            requests.post(
                                ...
                                data={
                                    "from": f"BookaRide System <{sender_email}>",
                                    "to": admin_email,
                                    "subject": f"Ã¢Å“â€¦ Driver {driver_name} Acknowledged Job #{booking_ref}",
                                    "html": html_content
                                }
                            )
                    except Exception as email_error:
...
async def create_tracking_session(data: TrackingSessionCreate, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ Tracking session created for booking {data.bookingId} - Ref: {tracking_ref}")
            ...
            if customer_phone:
                try:
                    ...
                    if account_sid and auth_token and twilio_phone:
                        ...
                        sms_body = f"""Ã°Å¸Å¡â€” {driver_name} is on the way!
...
- BookaRide NZ"""
                        ...
                        logger.info(f"Ã°Å¸â€œÂ± Tracking link sent to customer: {formatted_phone}")
                        
                except Exception as sms_error:
                    logger.error(f"Error sending tracking SMS to customer: {sms_error}")
        
        logger.info(f"Ã°Å¸â€œÂ Driver started tracking - Session: {session_id}")
...
async def stop_driver_tracking(session_id: str):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ Driver stopped tracking - Session: {session_id}")
...
async def send_tracking_link_to_driver(booking_id: str, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        if account_sid and auth_token and twilio_phone:
            ...
            sms_body = f"""Ã°Å¸â€œÂ SHARE YOUR LOCATION
...
- BookaRide"""
            ...
            logger.info(f"Ã°Å¸â€œÂ± Tracking link sent to driver {driver.get('name')} at {formatted_phone}")
...
async def send_shuttle_route_to_driver(
    ...
    try:
        ...
        # Build SMS message
        pickup_summary = "\n".join([
            f"Ã¢â‚¬Â¢ {p['name']} ({p['passengers']}pax) - {p['address'][:30]}..."
            for p in route_data["pickupDetails"][:5]
        ])
        
        message = f"""Ã°Å¸Å¡Â SHUTTLE {date} {time}
{route_data['totalPassengers']} passengers, {route_data['totalBookings']} pickups

{pickup_summary}

Ã°Å¸â€œÂ ROUTE: {route_data['googleMapsUrl']}"""
                    ...
                    for idx, (booking, element) in enumerate(zip(bookings, elements)):
                        if element['status'] == 'OK':
                            ...
                            if duration_minutes <= 5 and booking['id'] not in location.notifiedPickups:
                                ...
                                logger.info(f"Ã°Å¸â€œÂ± Arriving soon SMS sent to {booking['name']} at {booking['phone']}")
                        else:
...
def send_arriving_soon_sms(booking: dict, eta_minutes: int, driver_name: str):
    ...
    try:
        ...
        # Build message
        message = f"""Ã°Å¸Å¡Â Your Book A Ride shuttle is arriving in ~{eta_minutes} minutes!

Please be ready at:
Ã°Å¸â€œÂ {booking.get('pickupAddress', 'your pickup location')}

Driver: {driver_name}
Passengers: {booking.get('passengers', 1)}

See you soon! Ã°Å¸â„¢â€š"""
        ...
        logger.info(f"Ã¢Å“â€¦ Arriving soon SMS sent to {booking.get('name')} at {formatted_phone}")
        return True
...
async def start_shuttle_run(date: str, time: str, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
            ...
            try:
                ...
                if notify_dt > now:
                    ...
                    if booking and not booking.get('arrivingSoonSent'):
                        ...
                        scheduled_count += 1
                        logger.info(f"Ã°Å¸â€œâ€¦ Scheduled SMS for {item['name']} at {notify_dt.strftime('%H:%M')}")
                else:
        ...
        logger.info(f"Ã°Å¸Å¡Â Shuttle started: {date} {time} - {scheduled_count} notifications scheduled")
...
async def send_scheduled_arriving_sms(booking_id: str):
    ...
    try:
        ...
        if success:
            ...
            logger.info(f"Ã°Å¸â€œÂ± Scheduled SMS sent to {booking.get('name')}")
    except Exception as e:
...
async def assign_shuttle_driver(
    ...
    try:
        ...
        if driver_phone:
            ...
            if twilio_sid and twilio_token and twilio_phone:
                pickup_summary = "\n".join([
                    f"Ã¢â‚¬Â¢ {b['name']} ({b['passengers']}pax)"
                    for b in bookings[:5]
                ])
                
                route_message = f"""Ã°Å¸Å¡Â SHUTTLE ASSIGNED - {date} {time}

{len(bookings)} pickups, {sum(b['passengers'] for b in bookings)} passengers

{pickup_summary}

Ã°Å¸â€œÂ OPEN ROUTE IN MAPS:
{maps_url}

Customers will be auto-notified 5 mins before their pickup. Drive safe! Ã°Å¸â„¢â€š"""
                ...
                try:
                    ...
                    logger.info(f"Ã°Å¸â€œÂ± Route sent to driver {driver_name} at {formatted_driver_phone}")
                except Exception as sms_error:
                ...
                for idx, booking in enumerate(bookings):
                    ...
                    if notify_dt > now:
                        if not booking.get('arrivingSoonSent'):
                            ...
                            scheduled_count += 1
                            logger.info(f"Ã°Å¸â€œâ€¦ Scheduled SMS for {booking['name']} at {notify_dt.strftime('%H:%M')}")
                    else:
        ...
        logger.info(f"Ã°Å¸Å¡Â Driver {driver_name} assigned to shuttle {date} {time} - {scheduled_count} SMS scheduled")
...
async def auto_sync_from_production():
    ...
    try:
        logger.info("Ã°Å¸â€â€ž Auto-sync: Starting sync from production...")
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Ã°Å¸â€â€ž Auto-sync complete: {bookings_synced} bookings, {drivers_synced} drivers")
    ...
    try:
        ...
        if data['status'] == 'OK' and len(data['rows']) > 0:
            ...
            if element['status'] == 'OK':
                ...
                logger.info(f"Ã°Å¸â€œÂ Drive time from base to {destination_address[:50]}...: {drive_minutes} mins")
...
async def check_return_booking_alerts():
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€â€ [return_alerts] Checking return bookings - NZ time: {now_nz.strftime('%Y-%m-%d %H:%M')}")
                ...
                if alert_time <= now_nz < alert_window_end:
                    ...
                    if twilio_sid and twilio_token and twilio_from:
                        try:
                            ...
                            sms_body = f"Ã¢Å¡Â Ã¯Â¸Â RETURN TRIP ALERT!\n"
                            ...
                            logger.info(f"Ã¢Å“â€¦ Return alert SMS sent for booking #{booking_ref}")
                            alerts_sent += 1
                    ...
                    if driver_id:
                        ...
                        if driver and driver.get('phone'):
                            try:
                                driver_sms = f"Ã°Å¸Å¡â€” RETURN PICKUP REMINDER\n"
                                ...
                                logger.info(f"Ã¢Å“â€¦ Driver return reminder sent for booking #{booking_ref}")
        ...
        logger.info(f"Ã°Å¸â€â€ [return_alerts] Check complete: {alerts_sent} alerts sent")
...
async def create_afterpay_checkout(request: AfterpayCheckoutRequest):
    ...
    try:
        ...
        logger.info(f"Ã¢Å“â€¦ Afterpay checkout created: token={token}, booking={request.booking_id}")
    ...
    try:
        ...
        if transaction:
            ...
            if status == "APPROVED":
                ...
                logger.info(f"Ã¢Å“â€¦ Afterpay payment captured: order_id={afterpay_order_id}, booking={transaction['booking_id']}")
...
async def sync_all_contacts_to_icloud(current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ± iCloud bulk sync complete: {synced} synced, {failed} failed, {skipped} skipped")
            ...
            <html>
                <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="padding: 20px; background-color: #f5f5f5;">
                        <h2>Ã°Å¸Å¡â€” Driver Application Received</h2>
...
async def confirm_driver_acknowledgment(booking_id: str, trip_type: str = "outbound", current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        booking_ref = get_booking_reference(booking)
        logger.info(f"Ã¢Å“â€¦ Admin manually confirmed {trip_type} driver {driver_name} for booking #{booking_ref}")
...
To rebook: bookaride.co.nz"""
    ...
    logger.info(f"Ã°Å¸â€œÂ± Sending cancellation SMS to: {formatted_phone}")
    ...
    logger.info(f"Ã¢Å“â€¦ Cancellation SMS sent to {formatted_phone} - SID: {message.sid}")
...
async def initialize_all_seo_pages(current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        hibiscus_coast_suburbs = [
            ...
            {"slug": "whangaparaoa", "name": "WhangaparÃ„Âoa", "city": "Hibiscus Coast", "distance": 48, "price": 135},
...
async def import_bookings_from_csv(
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ¥ WordPress import: {imported} imported, {skipped} skipped, {len(errors)} errors")
...
async def quick_import_wordpress(request: Request):
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ¥ Quick WordPress import: {imported} imported, {skipped} skipped, {len(errors)} errors")
...
async def fix_imported_bookings():
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€Â§ Fixed imported bookings: {fixed_status} restored, {fixed_dates} dates corrected")
...
async def batch_sync_calendar_task(query: dict):
    ...
    try:
        logger.info("Ã°Å¸â€œâ€¦ Starting batch calendar sync for imported bookings...")
        ...
        async for booking in cursor:
            try:
                ...
                if success:
                    synced += 1
                    if synced % 50 == 0:
                        logger.info(f"Ã°Å¸â€œâ€¦ Calendar sync progress: {synced} bookings synced")
                else:
                    failed += 1
        ...
        logger.info(f"Ã°Å¸â€œâ€¦ Batch calendar sync completed: {synced} synced, {failed} failed")
...
async def auto_archive_completed_bookings():
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€œÂ¦ [Auto-Archive] Starting - NZ time: {now_nz.strftime('%Y-%m-%d %H:%M')}")
        ...
        logger.info(f"Ã°Å¸â€œÂ¦ [Auto-Archive] Found {len(completed_bookings)} completed bookings to check")
            ...
            try:
                ...
                if trip_end_date < today_str:
                    ...
                    if archived_count <= 5:  # Only log first 5 for brevity
                        logger.info(f"Ã°Å¸â€œÂ¦ Auto-archived: Ref #{booking.get('referenceNumber')} - {booking.get('name')} (trip ended: {trip_end_date})")
                else:
                    skipped_count += 1
                    
            except Exception as e:
                logger.error(f"Ã°Å¸â€œÂ¦ Error processing booking {booking.get('id', 'unknown')}: {str(e)}")
                continue
        
        logger.info(f"Ã°Å¸â€œÂ¦ [Auto-Archive] Completed - Archived: {archived_count}, Skipped: {skipped_count}")
        return {"archived": archived_count, "skipped": skipped_count, "date": today_str}
        
    except Exception as e:
        logger.error(f"Ã°Å¸â€œÂ¦ [Auto-Archive] Error: {str(e)}")
...
async def interval_reminder_check():
    ...
    try:
        ...
        if current_hour < 8 or current_hour > 22:
            logger.debug(f"Ã°Å¸â€â€ [interval_check] Outside reminder hours ({current_hour}:00 NZ), skipping")
            return
        ...
        logger.error(f"Ã¢ÂÅ’ Interval reminder check error: {str(e)}")
...
async def startup_reminder_check():
    ...
    try:
        ...
        logger.info(f"Ã°Å¸Å¡â‚¬ Startup reminder check - NZ time: {nz_now.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        logger.error(f"Ã¢ÂÅ’ Startup reminder check error: {str(e)}")
...
async def send_arrival_pickup_emails():
    ...
    try:
        ...
        logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â [Arrival Emails] Checking for airport arrivals on {tomorrow}")
        ...
        if not bookings:
            logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â [Arrival Emails] No airport arrivals found for {tomorrow}")
            return {"sent": 0, "date": tomorrow}
        
        logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â [Arrival Emails] Found {len(bookings)} airport arrivals for {tomorrow}")
        ...
        if not mailgun_api_key or not mailgun_domain:
            logger.warning("Ã¢Å“Ë†Ã¯Â¸Â [Arrival Emails] Mailgun not configured")
        ...
        for booking in bookings:
            try:
                ...
                if booking.get('arrivalEmailSent'):
                    logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â Skipping {email} - arrival email already sent")
                    continue
                ...
                response = requests.post(
                    ...
                    data={
                        ...
                        "to": email,
                        "subject": f"Ã¢Å“Ë†Ã¯Â¸Â Your Airport Pickup Tomorrow - Where to Meet Your Driver",
                        "html": email_html
                    }
                )
                ...
                if response.status_code == 200:
                    ...
                    sent_count += 1
                    logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â Arrival email sent to {email} for booking {booking.get('id')}")
                else:
                    logger.error(f"Ã¢Å“Ë†Ã¯Â¸Â Failed to send arrival email to {email}: {response.text}")
                    
            except Exception as e:
                logger.error(f"Ã¢Å“Ë†Ã¯Â¸Â Error sending arrival email: {str(e)}")
                continue
        
        logger.info(f"Ã¢Å“Ë†Ã¯Â¸Â [Arrival Emails] Completed - {sent_count} emails sent for {tomorrow}")
        return {"sent": sent_count, "date": tomorrow}
        
    except Exception as e:
        logger.error(f"Ã¢Å“Ë†Ã¯Â¸Â [Arrival Emails] Error: {str(e)}")
...
async def run_daily_error_check():
    ...
    try:
        ...
        logger.info(f"Ã°Å¸â€Â [Daily Error Check] Starting - NZ time: {now_nz.strftime('%Y-%m-%d %H:%M')}")
        ...
        for booking in upcoming_bookings:
            ...
            if not booking.get('name'):
                issues.append(f"Ã°Å¸Å¡Â¨ Booking #{booking_ref}: Missing customer NAME")
            if not booking.get('phone'):
                issues.append(f"Ã°Å¸Å¡Â¨ Booking #{booking_ref}: Missing customer PHONE")
            if not booking.get('email'):
                warnings.append(f"Ã¢Å¡Â Ã¯Â¸Â Booking #{booking_ref}: Missing customer EMAIL")
            
            # Check for missing addresses
            if not booking.get('pickupAddress'):
                issues.append(f"Ã°Å¸Å¡Â¨ Booking #{booking_ref}: Missing PICKUP address")
            if not booking.get('dropoffAddress'):
                issues.append(f"Ã°Å¸Å¡Â¨ Booking #{booking_ref}: Missing DROPOFF address")
            
            # Check for missing time
            if not booking.get('time'):
                issues.append(f"Ã°Å¸Å¡Â¨ Booking #{booking_ref}: Missing PICKUP TIME")
            
            # Check for unassigned TODAY bookings
            if booking.get('date') == today_str:
                if not booking.get('driver_id') and not booking.get('assignedDriver'):
                    issues.append(f"Ã°Å¸Å¡Â¨ TODAY Booking #{booking_ref} ({booking.get('name')}): NO DRIVER ASSIGNED!")
        ...
        for booking in return_bookings:
            ...
            if not booking.get('returnDate'):
                warnings.append(f"Ã¢Å¡Â Ã¯Â¸Â Booking #{booking_ref}: Has bookReturn=true but missing RETURN DATE")
            if not booking.get('returnTime'):
                warnings.append(f"Ã¢Å¡Â Ã¯Â¸Â Booking #{booking_ref}: Has bookReturn=true but missing RETURN TIME")
        ...
        for booking in unpaid_today:
            ...
            if total_price and float(total_price) > 0:
                warnings.append(f"Ã¢Å¡Â Ã¯Â¸Â TODAY Booking #{booking_ref} ({booking.get('name')}): UNPAID - ${total_price}")
        ...
        for booking in recent_bookings:
            booking_ref = get_booking_reference(booking)
            warnings.append(f"Ã¢Å¡Â Ã¯Â¸Â Booking #{booking_ref}: Confirmation may not have been sent")
        ...
        for booking in bookings_needing_calendar:
            booking_ref = get_booking_reference(booking)
            issues.append(f"Ã°Å¸Å¡Â¨ Booking #{booking_ref} ({booking.get('name')}): NOT synced to CALENDAR!")
        ...
        for booking in return_trips_soon:
            ...
            if not has_return_calendar:
                ...
                issues.append(f"Ã°Å¸Å¡Â¨ {urgency} RETURN #{booking_ref} ({booking.get('name')}): {return_date} @ {return_time} - NOT in calendar!")
            ...
            if return_date == today_str:
                if not booking.get('return_driver_id') and not booking.get('driver_id'):
                    issues.append(f"Ã°Å¸Å¡Â¨ TODAY RETURN #{booking_ref} ({booking.get('name')}): @ {return_time} - NO DRIVER!")
        ...
        if active_drivers == 0:
            issues.append("Ã°Å¸Å¡Â¨ CRITICAL: No active drivers in system!")
...
Ã¢â€¢â€˜           BOOKARIDE DAILY ERROR CHECK REPORT                 Ã¢â€¢â€˜
Ã¢â€¢â€˜                    {report_time}                               Ã¢â€¢â€˜
...
Ã°Å¸â€œÅ  SUMMARY
Ã¢â‚¬Â¢ Bookings checked (today/tomorrow): {stats['total_bookings_checked']}
Ã¢â‚¬Â¢ Total bookings in database: {total_bookings}
Ã¢â‚¬Â¢ Active drivers: {active_drivers} / {total_drivers}
Ã¢â‚¬Â¢ Critical issues found: {stats['issues_found']}
Ã¢â‚¬Â¢ Warnings found: {stats['warnings_found']}

"""
        
        if issues:
            report += "Ã°Å¸Å¡Â¨ CRITICAL ISSUES (Require immediate attention):\n"
            report += "Ã¢â€â‚¬" * 50 + "\n"
        ...
        if warnings:
            report += "Ã¢Å¡Â Ã¯Â¸Â WARNINGS (Review recommended):\n"
            report += "Ã¢â€â‚¬" * 50 + "\n"
        ...
        if not issues and not warnings:
            report += "Ã¢Å“â€¦ ALL SYSTEMS HEALTHY - No issues detected!\n"
        
        logger.info(f"Ã°Å¸â€Â [Daily Error Check] Complete: {stats['issues_found']} issues, {stats['warnings_found']} warnings")
        ...
        try:
            ...
            if mailgun_api_key and mailgun_domain:
                subject = f"{'Ã°Å¸Å¡Â¨ ISSUES FOUND' if issues else 'Ã¢Å“â€¦ All Clear'} - BookaRide Daily Check {report_time}"
                ...
                if response.status_code == 200:
                    logger.info(f"Ã°Å¸â€Â [Daily Error Check] Email report sent to {admin_email}")
                else:
                    logger.error(f"Ã°Å¸â€Â [Daily Error Check] Failed to send email: {response.text}")
        except Exception as email_err:
            logger.error(f"Ã°Å¸â€Â [Daily Error Check] Email error: {str(email_err)}")
        ...
        if issues:
            try:
                ...
                if twilio_sid and twilio_token and twilio_from:
                    ...
                    sms_body = f"Ã°Å¸Å¡Â¨ BookaRide Daily Check\n"
                    ...
                    logger.info(f"Ã°Å¸â€Â [Daily Error Check] Critical alert SMS sent")
            except Exception as sms_err:
                logger.error(f"Ã°Å¸â€Â [Daily Error Check] SMS error: {str(sms_err)}")
        ...
        logger.error(f"Ã°Å¸â€Â [Daily Error Check] Fatal error: {str(e)}")
        return {"success": False, "error": str(e)}
    ...
    <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f3f4f6;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); border-radius: 16px 16px 0 0; padding: 30px; text-align: center;">
                <h1 style="color: white; margin: 0; font-size: 24px;">Ã¢Å“Ë†Ã¯Â¸Â Your Airport Pickup Tomorrow</h1>
            ...
            <div style="background: white; padding: 30px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                ...
                <div style="border: 2px solid {pickup_info['color']}; border-radius: 12px; overflow: hidden; margin-bottom: 25px;">
                    ...
                    <div style="padding: 20px;">
                        <h3 style="color: #111827; margin: 0 0 15px 0; font-size: 16px;">Ã°Å¸â€œÂ How to Find Your Driver:</h3>
                        <table style="width: 100%; border-collapse: collapse;">
                            {instructions_html}
                        </table>
                    </div>
                ...
                <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 12px; padding: 15px; margin-bottom: 25px; text-align: center;">
                    <p style="color: #92400e; margin: 0; font-size: 16px;">
                        Ã°Å¸â€˜â‚¬ <strong>Look for your name!</strong><br>
            ...
            <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px;">
                <p style="margin: 0;">BookaRide NZ Ã¢â‚¬Â¢ Premium Airport Transfers</p>
...
async def startup_event():
    ...
    if not default_admin:
        ...
        logger.info("Ã¢Å“â€¦ Default admin user created")
    else:
        ...
        logger.info("Ã¢Å“â€¦ Admin password reset and email updated to info@bookaride.co.nz")
    ...
    try:
        ...
        logger.info("Ã¢Å“â€¦ Database indexes created for faster queries")
    except Exception as e:
    ...
    try:
        ...
        logger.info("Ã¢Å“â€¦ Archive indexes created")
    except Exception as e:
    ...
    scheduler.start()
    logger.info("Ã°Å¸Å¡â‚¬ Scheduler started with all jobs:")
    logger.info("   Ã¢Å“â€œ Reminder: 8:00 AM NZ daily (primary)")
    logger.info("   Ã¢Å“â€œ Arrival emails: 9:00 AM NZ daily")
    logger.info("   Ã¢Å“â€œ Reminder: Hourly backup check")
    logger.info("   Ã¢Å“â€œ Abandoned bookings: Every 30 mins")
    logger.info("   Ã¢Å“â€œ Auto-sync: Every 5 minutes")
    logger.info("   Ã¢Å“â€œ Return alerts: Every 15 minutes")
    logger.info("   Ã¢Å“â€œ Daily error check: 6:00 AM NZ daily")
    logger.info("   Ã¢Å“â€œ Auto-archive: 2:00 AM NZ daily")
    logger.info("   Ã¢Å“â€œ Startup reminder check (running now...)")
