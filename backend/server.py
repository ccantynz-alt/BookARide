...
try:
    from email_sender import send_email as send_email_unified, get_noreply_email, is_email_configured, get_configured_provider
except ImportError:
    ...
    get_configured_provider = lambda: "None"
...
async def request_password_reset(reset_request: PasswordResetRequest):
    ...
    try:
        ...
        # Send reset email via unified sender (Gmail API -> SMTP -> Mailgun)
        ...
        if True:  # Build email and send via unified sender
            ...
            if send_email_unified and send_email_unified(email, "Password Reset Request - Book A Ride NZ Admin", email_html):
                ...
                logger.warning("Password reset email NOT sent - configure Gmail API, SMTP, or Mailgun (see GOOGLE_WORKSPACE_EMAIL_SETUP.md)")
...
async def send_booking_email(email_data: dict, current_admin: dict = Depends(get_current_admin)):
    try:
        ...
        # Send via unified email sender (Gmail API → SMTP → Mailgun)
        cc_str = cc_emails.strip() if cc_emails and cc_emails.strip() else None
...
async def send_booking_to_admin(booking_id: str, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        # Send via unified email sender (Gmail API -> SMTP -> Mailgun)
        admin_subject = f"Booking Details - {booking.get('name', 'Customer')} - {booking.get('id', '')[:8].upper()}"
        success = send_email_unified(
...
def send_booking_confirmation_email(booking: dict, include_payment_link: bool = True):
    """Send booking confirmation email via unified sender (Gmail API -> SMTP -> Mailgun)"""
    try:
        booking_ref = get_booking_reference(booking)
        ...
        cc_str = cc_email.strip() if cc_email and cc_email.strip() else None
        ...
        if success:
            cc_info = f" (CC: {cc_email})" if cc_str else ""
...
Write a professional, helpful response:"""
        ...
        # Send the AI-generated response via unified email sender
        ...
        # Send via unified sender (Gmail API -> SMTP -> Mailgun)
        email_success = send_email_unified(
        ...
        if email_success:
            ...
            logger.error(f"Failed to send auto-reply to {reply_to_email}")
    ...
    try:
        ...
        for booking in abandoned:
            try:
                ...
                recovery_success = send_email_unified(
                    to_email=email,
                    subject=subject,
                    html_content=html_content,
                    from_name="BookaRide NZ",
            ...
            for admin_email in admin_emails:
                try:
                    if send_email_unified(
                        ...
                        logger.error(f"Failed to send to {admin_email}")
                except Exception as e:
                    logger.error(f"Error sending to {admin_email}: {e}")
...
async def send_driver_notification(booking: dict, driver: dict, trip_type: str = "OUTBOUND"):
    ...
    try:
        ...
        if True:  # Always attempt via unified email sender
...
"""
            ...
            driver_email_success = send_email_unified(
                to_email=driver.get('email'),
...
async def send_cancellation_email(booking: dict, to_email: str, customer_name: str):
    """Send cancellation email via unified sender (Gmail API -> SMTP -> Mailgun)"""
    ...
    try:
        ...
        for booking in bookings:
            try:
                ...
                arrival_success = send_email_unified(
                    to_email=email,
...
"""
        ...
        try:
            ...
            report_success = send_email_unified(
                to_email=admin_email,
                subject=subject,
            ...
            if report_success:
                ...
                logger.error(f"[Daily Error Check] Failed to send email to {admin_email}")
