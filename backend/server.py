...
def run_async_task(coro_func, arg, task_description="background task"):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"âœ… Background task completed: {task_description}")
        finally:
            loop.close()
    except Exception as e:
        logger.error(f"âŒ Background task failed ({task_description}): {str(e)}")
...
def validate_booking_date(self):       
    ...
    if self.date:  # âœ… NOW PROPERLY INSIDE THE FUNCTION
        try:
            nz_tz = pytz.timezone('Pacific/Auckland')
            today = datetime.now(nz_tz).strftime('%Y-%m-%d')
            if self.date < today:
            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <div class="warning">
                            â° <strong>This link will expire in 1 hour.</strong><br>
    ...
    try:
        ...
        if google_api_key:
            ...
            if len(all_pickups) > 1:
                ...
                if data['status'] == 'OK' and len(data['routes']) > 0:
                    ...
                    logger.info(f"Multi-stop route: {len(all_pickups)} pickups â†’ dropoff, total: {distance_km}km")
                else:
        ...
        if is_concert_trip:
            if is_from_hibiscus_coast or is_to_hibiscus_coast:
                ...
                logger.info(f"ðŸŽµ CONCERT PRICING: Hibiscus Coast to Matakana Country Park - flat ${matakana_concert_base}")
            else:
                # From elsewhere - calculate distance to Hibiscus Coast and add $550
                logger.info(f"ðŸŽµ CONCERT PRICING: {distance_km}km total, charging to Hibiscus Coast + ${matakana_concert_base} concert transfer")
        ...
        # Calculate base price: distance Ã— rate for that bracket
        ...
        if is_concert_trip:
            if is_from_hibiscus_coast or is_to_hibiscus_coast:
                ...
                if total_price < matakana_concert_base:
                    logger.info(f"ðŸŽµ Hibiscus Coast â†’ Matakana Country Park: applying flat ${matakana_concert_base}")
                ...
                logger.info(f"ðŸŽµ Auckland â†’ Matakana Country Park: {distance_to_hibiscus}km @ ${rate_to_hibiscus}/km = ${price_to_hibiscus:.2f} + ${matakana_concert_base} = ${total_price:.2f}")
...
async def quick_approve_booking(booking_id: str, action: str = "approve"):
    ...
    try:
        ...
        if not booking:
            return HTMLResponse(content="""
                <html><body style="font-family: Arial; text-align: center; padding: 50px;">
                    <h1 style="color: #DC2626;">âŒ Booking Not Found</h1>
        ...
        if action == "approve":
            ...
            logger.info(f"âœ… Booking {booking_ref} APPROVED via email quick-approve")
            ...
            return HTMLResponse(content=f"""
                <html><body style="font-family: Arial; text-align: center; padding: 50px; background-color: #f0fdf4;">
                    <div style="max-width: 500px; margin: 0 auto; background: white; padding: 40px; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <h1 style="color: #16a34a; font-size: 48px; margin: 0;">âœ…</h1>
            ...
            logger.info(f"âŒ Booking {booking_ref} REJECTED via email quick-approve")
            
            return HTMLResponse(content=f"""
                <html><body style="font-family: Arial; text-align: center; padding: 50px; background-color: #fef2f2;">
                    <div style="max-width: 500px; margin: 0 auto; background: white; padding: 40px; border-radius: 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                        <h1 style="color: #DC2626; font-size: 48px; margin: 0;">âŒ</h1>
            ...
            return HTMLResponse(content="""
                <html><body style="font-family: Arial; text-align: center; padding: 50px;">
                    <h1 style="color: #f59e0b;">âš ï¸ Invalid Action</h1>
        ...
        return HTMLResponse(content=f"""
            <html><body style="font-family: Arial; text-align: center; padding: 50px;">
                <h1 style="color: #DC2626;">âŒ Error</h1>
        ...
        if any(key in update_data for key in ['date', 'time', 'returnDate', 'returnTime', 'pickupAddress', 'dropoffAddress']):
            try:
                ...
                if updated_booking and updated_booking.get('calendar_event_id'):
                    ...
                    logger.info(f"ðŸ“… Calendar event updated for booking {booking_id}")
            except Exception as cal_error:
                logger.warning(f"âš ï¸ Failed to update calendar event: {str(cal_error)}")
    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <div style="padding: 20px; background-color: #ffffff; border: 1px solid #e8e4d9; border-top: none;">
                    <h2 style="color: #333; margin-top: 0;">ðŸ“‹ Booking Details</h2>
                    ...
                    <h3 style="color: #333; margin-top: 30px;">ðŸ‘¤ Customer Information</h3>
                    ...
                    <h3 style="color: #333; margin-top: 30px;">ðŸš— Trip Details</h3>
                    <div style="background-color: #faf8f3; padding: 15px; border-radius: 8px; margin: 15px 0;">
                    ...
                    <h3 style="color: #333; margin-top: 30px;">ðŸ’° Pricing Details</h3>
                    <div style="background-color: #faf8f3; padding: 15px; border-radius: 8px; margin: 15px 0;">
        ...
        response = requests.post(
            ...
            data={
                ...
                "to": admin_email,
                "subject": f"ðŸ“‹ Booking Details - {booking.get('name', 'Customer')} - {booking.get('id', '')[:8].upper()}",
                "html": html_content
            }
        )
...
EMAIL_TRANSLATIONS = {
    ...
    'zh': {
        'subject': 'é¢„è®¢ç¡®è®¤',
        'confirmed': 'é¢„è®¢å·²ç¡®è®¤ï¼',
        'greeting': 'å°Šæ•¬çš„',
        'intro': 'æ‚¨çš„è¡Œç¨‹å·²ç¡®è®¤ã€‚ä»¥ä¸‹æ˜¯æ‚¨çš„é¢„è®¢è¯¦æƒ…ï¼š',
        'reference': 'é¢„è®¢ç¼–å·',
        'service': 'æœåŠ¡ç±»åž‹',
        'pickup': 'ä¸Šè½¦åœ°ç‚¹',
        'dropoff': 'ä¸‹è½¦åœ°ç‚¹',
        ...
        'passengers': 'ä¹˜å®¢äººæ•°',
        'total': 'æ€»è´¹ç”¨',
        'contact_intro': 'æˆ‘ä»¬ä¼šåœ¨æŽ¥é€æ—¶é—´å‰è”ç³»æ‚¨ç¡®è®¤æ‰€æœ‰ç»†èŠ‚ã€‚',
        'contact': 'å¦‚æœ‰ä»»ä½•é—®é¢˜ï¼Œè¯·è”ç³»æˆ‘ä»¬',
        'or_call': 'æˆ–è‡´ç”µ',
        'thanks': 'æ„Ÿè°¢æ‚¨é€‰æ‹©BookaRideï¼'
    },
    'ja': {
        'subject': 'äºˆç´„ç¢ºèª',
        'confirmed': 'ã”äºˆç´„ãŒç¢ºå®šã—ã¾ã—ãŸï¼',
        'greeting': 'æ§˜',
        'intro': 'ã”äºˆç´„ãŒç¢ºå®šã—ã¾ã—ãŸã€‚äºˆç´„è©³ç´°ã¯ä»¥ä¸‹ã®é€šã‚Šã§ã™ï¼š',
        'reference': 'äºˆç´„ç•ªå·',
        'service': 'ã‚µãƒ¼ãƒ“ã‚¹ã‚¿ã‚¤ãƒ—',
        'pickup': 'ä¹—è»Šå ´æ‰€',
        'dropoff': 'é™è»Šå ´æ‰€',
        'date': 'æ—¥ä»˜',
        'time': 'æ™‚é–“',
        'passengers': 'ä¹—å®¢æ•°',
        'total': 'åˆè¨ˆé‡‘é¡',
        'contact_intro': 'ä¹—è»Šæ™‚é–“ãŒè¿‘ã¥ãã¾ã—ãŸã‚‰ã€è©³ç´°ç¢ºèªã®ãŸã‚ã”é€£çµ¡ã„ãŸã—ã¾ã™ã€‚',
        'contact': 'ã”è³ªå•ãŒã”ã–ã„ã¾ã—ãŸã‚‰ã€ãŠå•ã„åˆã‚ã›ãã ã•ã„',
        'or_call': 'ã¾ãŸã¯ãŠé›»è©±ãã ã•ã„',
        'thanks': 'BookaRideã‚’ã”åˆ©ç”¨ã„ãŸã ãã‚ã‚ŠãŒã¨ã†ã”ã–ã„ã¾ã™ï¼'
    },
    'ko': {
        'subject': 'ì˜ˆì•½ í™•ì¸',
        'confirmed': 'ì˜ˆì•½ì´ í™•ì •ë˜ì—ˆìŠµë‹ˆë‹¤!',
        'greeting': 'ê³ ê°ë‹˜ê»˜',
        'intro': 'ì˜ˆì•½ì´ í™•ì •ë˜ì—ˆìŠµë‹ˆë‹¤. ì˜ˆì•½ ìƒì„¸ ì •ë³´ëŠ” ë‹¤ìŒê³¼ ê°™ìŠµë‹ˆë‹¤:',
        'reference': 'ì˜ˆì•½ ë²ˆí˜¸',
        ...
        'passengers': 'ìŠ¹ê° ìˆ˜',
        'total': 'ì´ ê²°ì œ ê¸ˆì•¡',
        'contact_intro': 'íƒ‘ìŠ¹ ì‹œê°„ì´ ê°€ê¹Œì›Œì§€ë©´ ëª¨ë“  ì„¸ë¶€ ì‚¬í•­ì„ í™•ì¸í•˜ê¸° ìœ„í•´ ì—°ë½ë“œë¦¬ê² ìŠµë‹ˆë‹¤.',
        'contact': 'ë¬¸ì˜ì‚¬í•­ì´ ìžˆìœ¼ì‹œë©´ ì—°ë½ì£¼ì„¸ìš”',
        'or_call': 'ë˜ëŠ” ì „í™”ì£¼ì„¸ìš”',
        'thanks': 'BookaRideë¥¼ ì´ìš©í•´ ì£¼ì…”ì„œ ê°ì‚¬í•©ë‹ˆë‹¤!'
    },
    'fr': {
        'subject': 'Confirmation de RÃ©servation',
        'confirmed': 'RÃ©servation ConfirmÃ©e!',
        'greeting': 'Cher',
        'intro': 'Votre trajet a Ã©tÃ© confirmÃ©. Voici les dÃ©tails de votre rÃ©servation:',
        'reference': 'RÃ©fÃ©rence de RÃ©servation',
        'service': 'Type de Service',
        'pickup': 'Lieu de Prise en Charge',
        'dropoff': 'Lieu de DÃ©pose',
        'date': 'Date',
        'time': 'Heure',
        'passengers': 'Passagers',
        'total': 'Total PayÃ©',
        'contact_intro': 'Nous vous contacterons avant votre heure de prise en charge pour confirmer tous les dÃ©tails.',
        'contact': 'Pour toute question, veuillez nous contacter Ã ',
        'or_call': 'ou appelez',
        'thanks': 'Merci d\'avoir choisi BookaRide!'
    },
    'hi': {
        'subject': 'à¤¬à¥à¤•à¤¿à¤‚à¤— à¤ªà¥à¤·à¥à¤Ÿà¤¿',
        'confirmed': 'à¤¬à¥à¤•à¤¿à¤‚à¤— à¤•à¥€ à¤ªà¥à¤·à¥à¤Ÿà¤¿ à¤¹à¥‹ à¤—à¤ˆ!',
        'greeting': 'à¤ªà¥à¤°à¤¿à¤¯',
        'intro': 'à¤†à¤ªà¤•à¥€ à¤°à¤¾à¤‡à¤¡ à¤•à¥€ à¤ªà¥à¤·à¥à¤Ÿà¤¿ à¤¹à¥‹ à¤—à¤ˆ à¤¹à¥ˆà¥¤ à¤¯à¤¹à¤¾à¤‚ à¤†à¤ªà¤•à¥€ à¤¬à¥à¤•à¤¿à¤‚à¤— à¤µà¤¿à¤µà¤°à¤£ à¤¹à¥ˆà¤‚:',
        'reference': 'à¤¬à¥à¤•à¤¿à¤‚à¤— à¤¸à¤‚à¤¦à¤°à¥à¤­',
        'service': 'à¤¸à¥‡à¤µà¤¾ à¤ªà¥à¤°à¤•à¤¾à¤°',
        'pickup': 'à¤ªà¤¿à¤•à¤…à¤ª',
        'dropoff': 'à¤¡à¥à¤°à¥‰à¤ª-à¤‘à¤«',
        'date': 'à¤¤à¤¾à¤°à¥€à¤–',
        'time': 'à¤¸à¤®à¤¯',
        'passengers': 'à¤¯à¤¾à¤¤à¥à¤°à¥€',
        'total': 'à¤•à¥à¤² à¤­à¥à¤—à¤¤à¤¾à¤¨',
        'contact_intro': 'à¤¹à¤® à¤†à¤ªà¤•à¥‡ à¤ªà¤¿à¤•à¤…à¤ª à¤¸à¤®à¤¯ à¤¸à¥‡ à¤ªà¤¹à¤²à¥‡ à¤¸à¤­à¥€ à¤µà¤¿à¤µà¤°à¤£à¥‹à¤‚ à¤•à¥€ à¤ªà¥à¤·à¥à¤Ÿà¤¿ à¤•à¥‡ à¤²à¤¿à¤ à¤¸à¤‚à¤ªà¤°à¥à¤• à¤•à¤°à¥‡à¤‚à¤—à¥‡à¥¤',
        'contact': 'à¤¯à¤¦à¤¿ à¤†à¤ªà¤•à¥‡ à¤•à¥‹à¤ˆ à¤ªà¥à¤°à¤¶à¥à¤¨ à¤¹à¥ˆà¤‚, à¤¤à¥‹ à¤•à¥ƒà¤ªà¤¯à¤¾ à¤¹à¤®à¤¸à¥‡ à¤¸à¤‚à¤ªà¤°à¥à¤• à¤•à¤°à¥‡à¤‚',
        'or_call': 'à¤¯à¤¾ à¤•à¥‰à¤² à¤•à¤°à¥‡à¤‚',
        'thanks': 'BookaRide à¤šà¥à¤¨à¤¨à¥‡ à¤•à¥‡ à¤²à¤¿à¤ à¤§à¤¨à¥à¤¯à¤µà¤¾à¤¦!'
    }
}
...
def send_booking_confirmation_sms(booking: dict):
    ...
    try:
        ...
        if booking.get('bookReturn') or booking.get('returnDate'):
            ...
            return_text = f"\n\nðŸ”„ Return: {return_date} at {return_time}"
            if return_flight:
...
Thank you for booking with us!"""
        ...
        logger.info(f"ðŸ“± Sending confirmation SMS to: {formatted_phone}")
        ...
        logger.info(f"âœ… Confirmation SMS sent to {formatted_phone} - SID: {message.sid}")
        return True
...
def send_customer_confirmation(booking: dict):
    ...
    if booking_id:
        try:
            ...
            sync_client.close()
            logger.info(f"âœ… Confirmation status updated for booking {booking_id}")
        except Exception as e:
...
def send_reminder_email(booking: dict):
    ...
    try:
        ...
        if booking_status in ['cancelled', 'canceled', 'deleted']:
            logger.warning(f"âš ï¸ Skipping reminder email for CANCELLED booking: {booking.get('name')} (Ref: {booking.get('referenceNumber')})")
            return False
            ...
            airport_instructions = f'''
                    <div style="background: linear-gradient(135deg, #1a365d 0%, #2c5282 100%); border-radius: 12px; padding: 25px; margin: 25px 0; color: white;">
                        <h3 style="margin: 0 0 15px 0; font-size: 18px; display: flex; align-items: center;">
                            âœˆï¸ INTERNATIONAL ARRIVALS - MEETING POINT
                        </h3>
            ...
            <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
                ...
                <div style="padding: 25px;">
                    ...
                    <div style="background: #faf8f3; border-radius: 10px; padding: 20px; margin: 20px 0; border: 1px solid #e8e4d9;">
                        <h3 style="margin: 0 0 15px 0; color: #1a1a1a; font-size: 16px;">ðŸ“‹ BOOKING DETAILS</h3>
...
def send_reminder_sms(booking: dict):
    ...
    try:
        ...
        booking_status = booking.get('status', '').lower()
        if booking_status in ['cancelled', 'canceled', 'deleted']:
            logger.warning(f"âš ï¸ Skipping reminder SMS for CANCELLED booking: {booking.get('name')} (Ref: {booking.get('referenceNumber')})")
            return False
...
3. Walk to Allpress Espresso CafÃ©
4. All drivers wait in front of the cafÃ©
...
Questions? +64 21 743 321"""
        ...
        logger.info(f"ðŸ“± Sending reminder SMS to: {formatted_phone}")
        ...
        logger.info(f"âœ… Reminder SMS sent to {formatted_phone} - SID: {message.sid}")
        return True
...
async def track_flight(flight_number: str):
    ...
    try:
        ...
        logger.info(f"âœˆï¸ Flight {fn}: {status} - Arriving {arr_code} at {format_time(arr_actual)}")
...
async def handle_incoming_email(request: Request):
    ...
    try:
        ...
        logger.info(f"ðŸ“§ Incoming email from: {reply_to_email}, Subject: {subject}")
...
Write a professional, helpful response:"""
        ...
        if response.status_code == 200:
            logger.info(f"âœ… AI auto-reply sent to {reply_to_email}")
    ...
    try:
        ...
        for booking in abandoned:
            try:
                ...
                subject = "Complete Your BookaRide Booking ðŸš—"
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="background: #f5f5f5; padding: 20px; text-align: center; font-size: 12px; color: #666;">
                        <p>Â© BookaRide NZ | Auckland Airport Transfers</p>
                    </div>
                </div>
                """
...
- Use emojis sparingly but naturally ðŸš—
...
Respond helpfully and naturally as the BookaRide assistant:"""
        ...
        # Fallback response
        return {
            "response": "I apologize, I'm having a brief technical issue. For immediate assistance, please call us at 0800 BOOK A RIDE or visit bookaride.co.nz/book-now to make a booking. We're here to help! ðŸš—"
        }
...
async def send_daily_reminders_core(source: str = "unknown"):
    ...
    if reminder_lock.locked():
        logger.info(f"ðŸ”’ [{source}] Reminder job already running, skipping to prevent duplicates")
    ...
    async with reminder_lock:
        try:
            ...
            logger.info(f"ðŸ”” [{source}] Checking reminders - NZ time: {nz_now.strftime('%Y-%m-%d %H:%M:%S')}, Tomorrow: {nz_tomorrow}")
            ...
            logger.info(f"ðŸ”” [{source}] Found {len(bookings)} bookings needing reminders for {nz_tomorrow}")
            ...
            for booking in bookings:
                ...
                if update_result.modified_count == 0:
                    skipped_count += 1
                    logger.debug(f"â­ï¸ Skipping {booking_name} - already being processed by another job")
                    continue
                ...
                try:
                    ...
                    if booking.get('email'):
                        ...
                        if email_sent:
                            logger.info(f"âœ‰ï¸ Reminder email sent to {booking.get('email')}")
                    
                    # Send SMS reminder
                    if booking.get('phone'):
                        sms_sent = send_reminder_sms(booking)
                        if sms_sent:
                            logger.info(f"ðŸ“± Reminder SMS sent to {booking.get('phone')}")
                    ...
                    logger.error(f"âŒ Error sending reminder to {booking_name}: {str(send_error)}")
            ...
            logger.info(f"âœ… [{source}] Reminders complete: {sent_count} sent, {skipped_count} skipped")
            return {"success": True, "reminders_sent": sent_count, "skipped": skipped_count, "source": source}
            
        except Exception as e:
            logger.error(f"âŒ [{source}] Reminder error: {str(e)}")
            raise
...
async def send_booking_notification_to_admin(booking: dict):
    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <div style="background: linear-gradient(135deg, #D4AF37 0%, #B8960C 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
                    <h1 style="margin: 0;">BookaRide.co.nz</h1>
                    <p style="margin: 5px 0; font-size: 14px; color: rgba(255,255,255,0.9);">ðŸ”” New Booking Received</p>
                </div>
                ...
                <div style="padding: 20px; background-color: #ffffff; border: 1px solid #e8e4d9; border-top: none;">
                    ...
                    <div style="background-color: #faf8f3; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #D4AF37;">
                        ...
                        <p style="margin: 5px 0;"><strong>Pickup:</strong> {booking.get('pickupAddress', 'N/A')}</p>
                        <p style="margin: 5px 0;"><strong>Drop-off:</strong> {booking.get('dropoffAddress', 'N/A')}</p>
                        {'<hr style="border: 0; border-top: 1px solid #e8e4d9; margin: 15px 0;"><div style="background-color: #e3f2fd; padding: 10px; border-radius: 5px; border-left: 4px solid #2196F3;"><p style="margin: 0; font-weight: bold; color: #1565C0;">âœˆï¸ FLIGHT INFORMATION</p><p style="margin: 5px 0 0 0;"><strong>Flight Number:</strong> ' + (booking.get('flightNumber') or booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber') or 'N/A') + '</p></div>' if (booking.get('flightNumber') or booking.get('departureFlightNumber') or booking.get('arrivalFlightNumber')) else ''}
                        {'<hr style="border: 0; border-top: 1px solid #e8e4d9; margin: 15px 0;"><div style="background-color: #f3e5f5; padding: 10px; border-radius: 5px; border-left: 4px solid #9C27B0;"><p style="margin: 0; font-weight: bold; color: #7B1FA2;">ðŸ”„ RETURN TRIP BOOKED</p><p style="margin: 5px 0 0 0;"><strong>Return Date:</strong> ' + format_date_ddmmyyyy(booking.get('returnDate', '')) + ' at ' + format_time_ampm(booking.get('returnTime', '')) + '</p><p style="margin: 5px 0 0 0;"><strong>Return Flight:</strong> ' + (booking.get('returnFlightNumber') or booking.get('returnDepartureFlightNumber') or 'NOT PROVIDED - FOLLOW UP REQUIRED') + '</p></div>' if booking.get('bookReturn') or booking.get('returnDate') else ''}
                        <hr style="border: 0; border-top: 2px solid #D4AF37; margin: 15px 0;">
                    ...
                    <div style="margin-top: 20px; padding: 15px; background-color: #fff8e6; border-radius: 8px; border-left: 4px solid #D4AF37;">
                        <p style="margin: 0; font-weight: bold; color: #333;">âš¡ Action Required:</p>
        ...
        response = requests.post(
            ...
            data={
                "from": f"BookaRide System <{sender_email}>",
                "to": admin_email,
                "subject": f"ðŸ”” New Booking - {booking.get('name', 'Customer')} - {formatted_date} - Ref: {booking_ref}",
                "html": html_content
            }
        )
    ...
    try:
        ...
        if twilio_sid and twilio_token and twilio_from:
            ...
            sms_body = f"ðŸ”” NEW BookaRide: {booking.get('name')} booked for {formatted_date} {formatted_time}. Ref: #{booking_ref}. Check admin dashboard."
            ...
            logger.info(f"âœ… Admin SMS sent for new booking: #{booking_ref}")
            sms_sent = True
        else:
...
async def send_urgent_approval_notification(booking: dict):
    ...
    try:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                <div style="background: linear-gradient(135deg, #DC2626 0%, #B91C1C 100%); color: white; padding: 20px; text-align: center; border-radius: 10px 10px 0 0;">
                    <h1 style="margin: 0;">ðŸš¨ URGENT APPROVAL REQUIRED</h1>
                ...
                <div style="padding: 20px; background-color: #ffffff; border: 1px solid #fca5a5; border-top: none;">
                    <div style="background-color: #fef2f2; padding: 15px; border-radius: 8px; border-left: 4px solid #DC2626; margin-bottom: 20px;">
                        <p style="margin: 0; font-size: 18px; font-weight: bold; color: #991B1B;">âš ï¸ This booking requires your manual approval</p>
                    ...
                    <div style="background-color: #fff7ed; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f97316;">
                        <h3 style="margin-top: 0; color: #c2410c;">ðŸ“… Pickup Details</h3>
                    ...
                    <div style="margin-top: 20px; padding: 20px; background-color: #fef2f2; border-radius: 8px; border: 2px solid #DC2626; text-align: center;">
                        <p style="margin: 0; font-weight: bold; font-size: 16px; color: #991B1B;">ðŸ”” ACTION REQUIRED</p>
                        <p style="margin: 10px 0 0 0; font-size: 14px; color: #7F1D1D;">Approve or reject this booking:</p>
                        
                        <div style="margin-top: 20px;">
                            <a href="https://bookaride.co.nz/api/booking/quick-approve/{booking.get('id')}?action=approve" style="display: inline-block; margin: 5px; padding: 15px 40px; background-color: #16a34a; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">âœ… APPROVE</a>
                            <a href="https://bookaride.co.nz/api/booking/quick-approve/{booking.get('id')}?action=reject" style="display: inline-block; margin: 5px; padding: 15px 40px; background-color: #DC2626; color: white; text-decoration: none; border-radius: 8px; font-weight: bold; font-size: 16px;">âŒ REJECT</a>
                        </div>
        ...
        response = requests.post(
            ...
            data={
                ...
                "to": admin_email,
                "subject": f"ðŸš¨ URGENT APPROVAL - {booking.get('name', 'Customer')} - {formatted_date} {booking.get('time', '')} - Ref: {booking_ref}",
                "html": html_content
            }
        )
    ...
    try:
        ...
        if twilio_sid and twilio_token and twilio_from:
            ...
            sms_body = f"""ðŸš¨ URGENT BOOKING #{booking_ref}
...
âš ï¸ Reply YES to APPROVE or NO to DECLINE"""
            ...
            logger.info(f"âœ… Urgent SMS sent to admin {admin_phone} for booking: #{booking_ref} - SID: {message.sid}")
            ...
            try:
                ...
                sync_client.close()
                logger.info(f"âœ… Stored pending approval for booking #{booking_ref}")
...
async def send_driver_notification(booking: dict, driver: dict, trip_type: str = "OUTBOUND"):
    ...
    try:
        ...
        logger.info(f"ðŸ“§ send_driver_notification called for {trip_type} trip")
        logger.info(f"ðŸ“§ Booking data: name={booking.get('name')}, phone={booking.get('phone')}, pickup={booking.get('pickupAddress')}, date={booking.get('date')}, time={booking.get('time')}")
        logger.info(f"ðŸ“§ Driver data: name={driver.get('name')}, email={driver.get('email')}")
        ...
        logger.info(f"ðŸ“§ Formatted: date={formatted_date}, time={formatted_time}, ref={booking_ref}")
        ...
        if booking.get('driver_payout_override') is not None:
            ...
            logger.info(f"ðŸ“§ Using manual driver payout override: ${driver_payout:.2f}")
        else:
            ...
            if has_return:
                ...
                logger.info(f"ðŸ“§ Return booking: {trip_type} trip price = ${trip_price:.2f} (subtotal ${subtotal:.2f})")
            else:
            ...
            logger.info(f"ðŸ“§ Driver gets full subtotal (customer pays Stripe fee separately)")
            
            # Round to 2 decimal places
            driver_payout = round(driver_payout, 2)
        
        logger.info(f"ðŸ“§ Driver Payout: ${driver_payout:.2f} for {trip_type} trip")
            ...
            if booking.get('bookReturn'):
                ...
                if return_date:
                    ...
                    <div style="background-color: #f3e5f5; padding: 15px; border-radius: 8px; margin: 15px 0; border-left: 4px solid #9C27B0;">
                        <p style="margin: 0; font-weight: bold; color: #7B1FA2;">ðŸ”„ RETURN TRIP DETAILS</p>
                        ...
                        {'<p style="margin: 5px 0;"><strong>âœˆï¸ Return Flight:</strong> ' + return_flight + '</p>' if return_flight else '<p style="margin: 5px 0; color: #d32f2f;"><strong>âš ï¸ Return Flight:</strong> NOT PROVIDED</p>'}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto;">
        ...
        <tr>
            <td style="padding: 20px; background-color: #f5f5f5;">
                <h2 style="color: #1a1a1a; margin-top: 0;">âœ… New Booking Assignment - {trip_type} TRIP</h2>
                ...
                <table width="100%" cellpadding="15" cellspacing="0" style="background-color: white; border-radius: 8px; margin: 20px 0; border-left: 4px solid #D4AF37;">
                    <tr>
                        <td>
                            ...
                            <hr style="border: 0; border-top: 2px solid #D4AF37; margin: 15px 0;">
                            <p style="margin: 5px 0; font-size: 18px;"><strong>ðŸ’° Your Payout: ${driver_payout:.2f} NZD</strong></p>
                        </td>
                    </tr>
                </table>
                {return_html}
...
"""
            ...
            if response.status_code == 200:
                logger.info(f"âœ… Driver notification email sent to {driver.get('email')}")
                logger.info(f"ðŸ“§ Mailgun response: {response.text}")
            else:
                logger.error(f"âŒ Failed to send driver email: {response.status_code} - {response.text}")
        ...
        try:
            ...
            if account_sid and auth_token and twilio_phone:
                ...
                if driver_phone:
                    logger.info(f"ðŸ“± Sending driver SMS to: {driver_phone} (original: {driver.get('phone')})")
                    ...
                    payment_display = {
                        'paid': 'âœ… PAID',
                        'cash': 'ðŸ’µ CASH',
                        'pay-on-pickup': 'ðŸš— PAY ON PICKUP',
                        'xero-invoiced': 'ðŸ“„ INVOICED',
                        'unpaid': 'âš ï¸ UNPAID'
                    ...
                    flight_text = f"\nâœˆï¸ Flight: {flight_num}" if flight_num else ""
...
âš ï¸ REPLY YES to confirm you received this job.
...
Check your email for full details."""
                    ...
                    logger.info(f"âœ… Driver notification SMS sent to {driver_phone} - SID: {message.sid}")
                else:
                    logger.warning(f"âš ï¸ Driver {driver.get('name')} has no phone number - SMS not sent")
            else:
                logger.warning("âš ï¸ Twilio credentials not configured for driver SMS")
        except Exception as sms_error:
            logger.error(f"âŒ Error sending driver SMS: {str(sms_error)}")
    ...
    try:
        ...
        if service_account_json:
            ...
            try:
                ...
                logger.info("âœ… Service account credentials loaded from environment variable")
                return creds
...
async def create_calendar_event(booking: dict):
    ...
    try:
        ...
        outbound_event = {
            'summary': f"{customer_name} â†’ {eng['dropoff'].split(',')[0]}" + (" + Return" if has_return else ""),
...
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”'''}
        ...
        if has_return and booking.get('returnDate') and booking.get('returnTime'):
            ...
            if return_nz_dt:
                ...
                return_event = {
                    'summary': f"{customer_name} â† Return from {eng['dropoff'].split(',')[0]}",
                                ...
                                if mailgun_key:
                                    requests.post(
                                        ...
                                        data={
                                            ...
                                            "subject": f"ðŸš Shuttle Booking Confirmed - {shuttle_booking['date']} {shuttle_booking['departureTime']}",
                                            "html": f"""
                                            ...
                                            <h3>Payment Info:</h3>
                                            <p>ðŸ’³ A hold of <strong>${shuttle_booking.get('totalEstimated', 100)}</strong> has been placed on your card.</p>
                                            <p>âœ… You will only be charged the <strong>final price</strong> when the shuttle arrives at the airport.</p>
                                            <p>ðŸ’° The more passengers on your shuttle, the cheaper everyone pays!</p>
...
async def twilio_sms_webhook(request: Request):
    ...
    try:
        ...
        logger.info(f"ðŸ“± Incoming SMS from {from_number}: {message_body}")
        ...
        if is_admin and message_body in ['YES', 'Y', 'YEP', 'CONFIRM', 'APPROVE', 'OK', 'OKAY', 'ACCEPTED']:
            ...
            if pending:
                ...
                logger.info(f"âœ… Admin approved booking #{booking_ref} ({customer_name}) via SMS")
                ...
                try:
                    ...
                    if twilio_sid and twilio_token:
                        client = Client(twilio_sid, twilio_token)
                        client.messages.create(
                            body=f"âœ… Booking #{booking_ref} for {customer_name} APPROVED! Customer confirmation sent.",
                            from_=twilio_from,
                            to=admin_phone
                        )
                except Exception as e:
            ...
            if pending:
                ...
                logger.info(f"âŒ Admin declined booking #{booking_ref} ({customer_name}) via SMS")
                ...
                try:
                    ...
                    if twilio_sid and twilio_token:
                        client = Client(twilio_sid, twilio_token)
                        client.messages.create(
                            body=f"âŒ Booking #{booking_ref} for {customer_name} DECLINED. Customer notified.",
                            from_=twilio_from,
                            to=admin_phone
                        )
                except Exception as e:
        ...
        if message_body in ['YES', 'Y', 'YEP', 'CONFIRM', 'CONFIRMED', 'OK', 'OKAY', 'ACCEPTED']:
            ...
            if driver:
                ...
                if booking:
                    ...
                    logger.info(f"âœ… Driver {driver_name} acknowledged {trip_type} trip for booking #{booking_ref} (+ {batch_result.modified_count} other bookings)")
                    ...
                    try:
                        ...
                        if account_sid and auth_token and twilio_phone:
                            client = Client(account_sid, auth_token)
                            client.messages.create(
                                body=f"âœ… Thanks {driver_name}! Job #{booking_ref} confirmed. Customer: {booking.get('name')} on {format_date_ddmmyyyy(booking.get('date'))} at {booking.get('time')}",
                                from_=twilio_phone,
                                to=from_number
                            )
                    except Exception as sms_error:
                        ...
                        if mailgun_api_key and mailgun_domain:
                            ...
                            <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
                                <div style="background: #22c55e; color: white; padding: 15px; border-radius: 8px 8px 0 0; text-align: center;">
                                    <h2 style="margin: 0;">âœ… Driver Acknowledged Job</h2>
                                </div>
                            ...
                            requests.post(
                                ...
                                data={
                                    "from": f"BookaRide System <{sender_email}>",
                                    "to": admin_email,
                                    "subject": f"âœ… Driver {driver_name} Acknowledged Job #{booking_ref}",
                                    "html": html_content
                                }
                            )
                    except Exception as email_error:
...
async def create_tracking_session(data: TrackingSessionCreate, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        logger.info(f"ðŸ“ Tracking session created for booking {data.bookingId} - Ref: {tracking_ref}")
            ...
            if customer_phone:
                try:
                    ...
                    if account_sid and auth_token and twilio_phone:
                        ...
                        sms_body = f"""ðŸš— {driver_name} is on the way!
...
- BookaRide NZ"""
                        ...
                        logger.info(f"ðŸ“± Tracking link sent to customer: {formatted_phone}")
                        
                except Exception as sms_error:
                    logger.error(f"Error sending tracking SMS to customer: {sms_error}")
        
        logger.info(f"ðŸ“ Driver started tracking - Session: {session_id}")
...
async def stop_driver_tracking(session_id: str):
    ...
    try:
        ...
        logger.info(f"ðŸ“ Driver stopped tracking - Session: {session_id}")
...
async def send_tracking_link_to_driver(booking_id: str, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        if account_sid and auth_token and twilio_phone:
            ...
            sms_body = f"""ðŸ“ SHARE YOUR LOCATION
...
- BookaRide"""
            ...
            logger.info(f"ðŸ“± Tracking link sent to driver {driver.get('name')} at {formatted_phone}")
                    ...
                    for idx, (booking, element) in enumerate(zip(bookings, elements)):
                        if element['status'] == 'OK':
                            ...
                            if duration_minutes <= 5 and booking['id'] not in location.notifiedPickups:
                                ...
                                logger.info(f"ðŸ“± Arriving soon SMS sent to {booking['name']} at {booking['phone']}")
                        else:
...
def send_arriving_soon_sms(booking: dict, eta_minutes: int, driver_name: str):
    ...
    try:
        ...
        # Build message
        message = f"""ðŸš Your Book A Ride shuttle is arriving in ~{eta_minutes} minutes!

Please be ready at:
ðŸ“ {booking.get('pickupAddress', 'your pickup location')}
...
See you soon! ðŸ™‚"""
        ...
        logger.info(f"âœ… Arriving soon SMS sent to {booking.get('name')} at {formatted_phone}")
        return True
...
async def start_shuttle_run(date: str, time: str, current_admin: dict = Depends(get_current_admin)):
    ...
    try:
            ...
            try:
                ...
                if notify_dt > now:
                    ...
                    if booking and not booking.get('arrivingSoonSent'):
                        ...
                        scheduled_count += 1
                        logger.info(f"ðŸ“… Scheduled SMS for {item['name']} at {notify_dt.strftime('%H:%M')}")
                else:
        ...
        logger.info(f"ðŸš Shuttle started: {date} {time} - {scheduled_count} notifications scheduled")
...
async def send_scheduled_arriving_sms(booking_id: str):
    ...
    try:
        ...
        if success:
            ...
            logger.info(f"ðŸ“± Scheduled SMS sent to {booking.get('name')}")
    except Exception as e:
...
async def assign_shuttle_driver(
    ...
    try:
        ...
        if driver_phone:
            ...
            if twilio_sid and twilio_token and twilio_phone:
                pickup_summary = "\n".join([
                    f"â€¢ {b['name']} ({b['passengers']}pax)"
                    for b in bookings[:5]
                ])
                
                route_message = f"""ðŸš SHUTTLE ASSIGNED - {date} {time}

{len(bookings)} pickups, {sum(b['passengers'] for b in bookings)} passengers

{pickup_summary}

ðŸ“ OPEN ROUTE IN MAPS:
{maps_url}

Customers will be auto-notified 5 mins before their pickup. Drive safe! ðŸ™‚"""
                ...
                try:
                    ...
                    logger.info(f"ðŸ“± Route sent to driver {driver_name} at {formatted_driver_phone}")
                except Exception as sms_error:
                ...
                for idx, booking in enumerate(bookings):
                    ...
                    if notify_dt > now:
                        if not booking.get('arrivingSoonSent'):
                            ...
                            scheduled_count += 1
                            logger.info(f"ðŸ“… Scheduled SMS for {booking['name']} at {notify_dt.strftime('%H:%M')}")
                    else:
        ...
        logger.info(f"ðŸš Driver {driver_name} assigned to shuttle {date} {time} - {scheduled_count} SMS scheduled")
...
async def auto_sync_from_production():
    ...
    try:
        logger.info("ðŸ”„ Auto-sync: Starting sync from production...")
        ...
        if response.status_code == 200:
            ...
            logger.info(f"ðŸ”„ Auto-sync complete: {bookings_synced} bookings, {drivers_synced} drivers")
    ...
    try:
        ...
        if data['status'] == 'OK' and len(data['rows']) > 0:
            ...
            if element['status'] == 'OK':
                ...
                logger.info(f"ðŸ“ Drive time from base to {destination_address[:50]}...: {drive_minutes} mins")
...
async def check_return_booking_alerts():
    ...
    try:
        ...
        logger.info(f"ðŸ”” [return_alerts] Checking return bookings - NZ time: {now_nz.strftime('%Y-%m-%d %H:%M')}")
                ...
                if alert_time <= now_nz < alert_window_end:
                    ...
                    if twilio_sid and twilio_token and twilio_from:
                        try:
                            ...
                            sms_body = f"âš ï¸ RETURN TRIP ALERT!\n"
                            ...
                            logger.info(f"âœ… Return alert SMS sent for booking #{booking_ref}")
                            alerts_sent += 1
                    ...
                    if driver_id:
                        ...
                        if driver and driver.get('phone'):
                            try:
                                driver_sms = f"ðŸš— RETURN PICKUP REMINDER\n"
                                ...
                                logger.info(f"âœ… Driver return reminder sent for booking #{booking_ref}")
        ...
        logger.info(f"ðŸ”” [return_alerts] Check complete: {alerts_sent} alerts sent")
...
async def create_afterpay_checkout(request: AfterpayCheckoutRequest):
    ...
    try:
        ...
        logger.info(f"âœ… Afterpay checkout created: token={token}, booking={request.booking_id}")
    ...
    try:
        ...
        if transaction:
            ...
            if status == "APPROVED":
                ...
                logger.info(f"âœ… Afterpay payment captured: order_id={afterpay_order_id}, booking={transaction['booking_id']}")
...
async def sync_all_contacts_to_icloud(current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        logger.info(f"ðŸ“± iCloud bulk sync complete: {synced} synced, {failed} failed, {skipped} skipped")
            ...
            <html>
                <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="padding: 20px; background-color: #f5f5f5;">
                        <h2>ðŸš— Driver Application Received</h2>
...
async def confirm_driver_acknowledgment(booking_id: str, trip_type: str = "outbound", current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        booking_ref = get_booking_reference(booking)
        logger.info(f"âœ… Admin manually confirmed {trip_type} driver {driver_name} for booking #{booking_ref}")
...
To rebook: bookaride.co.nz"""
    ...
    logger.info(f"ðŸ“± Sending cancellation SMS to: {formatted_phone}")
    ...
    logger.info(f"âœ… Cancellation SMS sent to {formatted_phone} - SID: {message.sid}")
...
async def initialize_all_seo_pages(current_admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        hibiscus_coast_suburbs = [
            ...
            {"slug": "whangaparaoa", "name": "WhangaparÄoa", "city": "Hibiscus Coast", "distance": 48, "price": 135},
...
async def import_bookings_from_csv(
    ...
    try:
        ...
        logger.info(f"ðŸ“¥ WordPress import: {imported} imported, {skipped} skipped, {len(errors)} errors")
...
async def quick_import_wordpress(request: Request):
    ...
    try:
        ...
        logger.info(f"ðŸ“¥ Quick WordPress import: {imported} imported, {skipped} skipped, {len(errors)} errors")
...
async def fix_imported_bookings():
    ...
    try:
        ...
        logger.info(f"ðŸ”§ Fixed imported bookings: {fixed_status} restored, {fixed_dates} dates corrected")
...
async def batch_sync_calendar_task(query: dict):
    ...
    try:
        logger.info("ðŸ“… Starting batch calendar sync for imported bookings...")
        ...
        async for booking in cursor:
            try:
                ...
                if success:
                    synced += 1
                    if synced % 50 == 0:
                        logger.info(f"ðŸ“… Calendar sync progress: {synced} bookings synced")
                else:
                    failed += 1
        ...
        logger.info(f"ðŸ“… Batch calendar sync completed: {synced} synced, {failed} failed")
...
async def auto_archive_completed_bookings():
    ...
    try:
        ...
        logger.info(f"ðŸ“¦ [Auto-Archive] Starting - NZ time: {now_nz.strftime('%Y-%m-%d %H:%M')}")
        ...
        logger.info(f"ðŸ“¦ [Auto-Archive] Found {len(completed_bookings)} completed bookings to check")
            ...
            try:
                ...
                if trip_end_date < today_str:
                    ...
                    if archived_count <= 5:  # Only log first 5 for brevity
                        logger.info(f"ðŸ“¦ Auto-archived: Ref #{booking.get('referenceNumber')} - {booking.get('name')} (trip ended: {trip_end_date})")
                else:
                    skipped_count += 1
                    
            except Exception as e:
                logger.error(f"ðŸ“¦ Error processing booking {booking.get('id', 'unknown')}: {str(e)}")
                continue
        
        logger.info(f"ðŸ“¦ [Auto-Archive] Completed - Archived: {archived_count}, Skipped: {skipped_count}")
        return {"archived": archived_count, "skipped": skipped_count, "date": today_str}
        
    except Exception as e:
        logger.error(f"ðŸ“¦ [Auto-Archive] Error: {str(e)}")
...
async def interval_reminder_check():
    ...
    try:
        ...
        if current_hour < 8 or current_hour > 22:
            logger.debug(f"ðŸ”” [interval_check] Outside reminder hours ({current_hour}:00 NZ), skipping")
            return
        ...
        logger.error(f"âŒ Interval reminder check error: {str(e)}")
...
async def startup_reminder_check():
    ...
    try:
        ...
        logger.info(f"ðŸš€ Startup reminder check - NZ time: {nz_now.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        logger.error(f"âŒ Startup reminder check error: {str(e)}")
...
async def send_arrival_pickup_emails():
    ...
    try:
        ...
        logger.info(f"âœˆï¸ [Arrival Emails] Checking for airport arrivals on {tomorrow}")
        ...
        if not bookings:
            logger.info(f"âœˆï¸ [Arrival Emails] No airport arrivals found for {tomorrow}")
            return {"sent": 0, "date": tomorrow}
        
        logger.info(f"âœˆï¸ [Arrival Emails] Found {len(bookings)} airport arrivals for {tomorrow}")
        ...
        if not mailgun_api_key or not mailgun_domain:
            logger.warning("âœˆï¸ [Arrival Emails] Mailgun not configured")
        ...
        for booking in bookings:
            try:
                ...
                if booking.get('arrivalEmailSent'):
                    logger.info(f"âœˆï¸ Skipping {email} - arrival email already sent")
                    continue
                ...
                response = requests.post(
                    ...
                    data={
                        ...
                        "to": email,
                        "subject": f"âœˆï¸ Your Airport Pickup Tomorrow - Where to Meet Your Driver",
                        "html": email_html
                    }
                )
                ...
                if response.status_code == 200:
                    ...
                    sent_count += 1
                    logger.info(f"âœˆï¸ Arrival email sent to {email} for booking {booking.get('id')}")
                else:
                    logger.error(f"âœˆï¸ Failed to send arrival email to {email}: {response.text}")
                    
            except Exception as e:
                logger.error(f"âœˆï¸ Error sending arrival email: {str(e)}")
                continue
        
        logger.info(f"âœˆï¸ [Arrival Emails] Completed - {sent_count} emails sent for {tomorrow}")
        return {"sent": sent_count, "date": tomorrow}
        
    except Exception as e:
        logger.error(f"âœˆï¸ [Arrival Emails] Error: {str(e)}")
...
async def run_daily_error_check():
    ...
    try:
        ...
        logger.info(f"ðŸ” [Daily Error Check] Starting - NZ time: {now_nz.strftime('%Y-%m-%d %H:%M')}")
        ...
        for booking in upcoming_bookings:
            ...
            if not booking.get('name'):
                issues.append(f"ðŸš¨ Booking #{booking_ref}: Missing customer NAME")
            if not booking.get('phone'):
                issues.append(f"ðŸš¨ Booking #{booking_ref}: Missing customer PHONE")
            if not booking.get('email'):
                warnings.append(f"âš ï¸ Booking #{booking_ref}: Missing customer EMAIL")
            
            # Check for missing addresses
            if not booking.get('pickupAddress'):
                issues.append(f"ðŸš¨ Booking #{booking_ref}: Missing PICKUP address")
            if not booking.get('dropoffAddress'):
                issues.append(f"ðŸš¨ Booking #{booking_ref}: Missing DROPOFF address")
            
            # Check for missing time
            if not booking.get('time'):
                issues.append(f"ðŸš¨ Booking #{booking_ref}: Missing PICKUP TIME")
            
            # Check for unassigned TODAY bookings
            if booking.get('date') == today_str:
                if not booking.get('driver_id') and not booking.get('assignedDriver'):
                    issues.append(f"ðŸš¨ TODAY Booking #{booking_ref} ({booking.get('name')}): NO DRIVER ASSIGNED!")
        ...
        for booking in return_bookings:
            ...
            if not booking.get('returnDate'):
                warnings.append(f"âš ï¸ Booking #{booking_ref}: Has bookReturn=true but missing RETURN DATE")
            if not booking.get('returnTime'):
                warnings.append(f"âš ï¸ Booking #{booking_ref}: Has bookReturn=true but missing RETURN TIME")
        ...
        for booking in unpaid_today:
            ...
            if total_price and float(total_price) > 0:
                warnings.append(f"âš ï¸ TODAY Booking #{booking_ref} ({booking.get('name')}): UNPAID - ${total_price}")
        ...
        for booking in recent_bookings:
            booking_ref = get_booking_reference(booking)
            warnings.append(f"âš ï¸ Booking #{booking_ref}: Confirmation may not have been sent")
        ...
        for booking in bookings_needing_calendar:
            booking_ref = get_booking_reference(booking)
            issues.append(f"ðŸš¨ Booking #{booking_ref} ({booking.get('name')}): NOT synced to CALENDAR!")
        ...
        for booking in return_trips_soon:
            ...
            if not has_return_calendar:
                ...
                issues.append(f"ðŸš¨ {urgency} RETURN #{booking_ref} ({booking.get('name')}): {return_date} @ {return_time} - NOT in calendar!")
            ...
            if return_date == today_str:
                if not booking.get('return_driver_id') and not booking.get('driver_id'):
                    issues.append(f"ðŸš¨ TODAY RETURN #{booking_ref} ({booking.get('name')}): @ {return_time} - NO DRIVER!")
        ...
        if active_drivers == 0:
            issues.append("ðŸš¨ CRITICAL: No active drivers in system!")
...
â•‘           BOOKARIDE DAILY ERROR CHECK REPORT                 â•‘
â•‘                    {report_time}                               â•‘
â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

ðŸ“Š SUMMARY
â€¢ Bookings checked (today/tomorrow): {stats['total_bookings_checked']}
â€¢ Total bookings in database: {total_bookings}
â€¢ Active drivers: {active_drivers} / {total_drivers}
â€¢ Critical issues found: {stats['issues_found']}
â€¢ Warnings found: {stats['warnings_found']}

"""
        
        if issues:
            report += "ðŸš¨ CRITICAL ISSUES (Require immediate attention):\n"
            report += "â”€" * 50 + "\n"
        ...
        if not issues and not warnings:
            report += "âœ… ALL SYSTEMS HEALTHY - No issues detected!\n"
        
        logger.info(f"ðŸ” [Daily Error Check] Complete: {stats['issues_found']} issues, {stats['warnings_found']} warnings")
        ...
        try:
            ...
            if mailgun_api_key and mailgun_domain:
                subject = f"{'ðŸš¨ ISSUES FOUND' if issues else 'âœ… All Clear'} - BookaRide Daily Check {report_time}"
                ...
                if response.status_code == 200:
                    logger.info(f"ðŸ” [Daily Error Check] Email report sent to {admin_email}")
                else:
                    logger.error(f"ðŸ” [Daily Error Check] Failed to send email: {response.text}")
        except Exception as email_err:
            logger.error(f"ðŸ” [Daily Error Check] Email error: {str(email_err)}")
        ...
        if issues:
            try:
                ...
                if twilio_sid and twilio_token and twilio_from:
                    ...
                    sms_body = f"ðŸš¨ BookaRide Daily Check\n"
                    ...
                    logger.info(f"ðŸ” [Daily Error Check] Critical alert SMS sent")
            except Exception as sms_err:
                logger.error(f"ðŸ” [Daily Error Check] SMS error: {str(sms_err)}")
        ...
        logger.error(f"ðŸ” [Daily Error Check] Fatal error: {str(e)}")
        return {"success": False, "error": str(e)}
    ...
    <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f3f4f6;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); border-radius: 16px 16px 0 0; padding: 30px; text-align: center;">
                <h1 style="color: white; margin: 0; font-size: 24px;">âœˆï¸ Your Airport Pickup Tomorrow</h1>
            ...
            <div style="background: white; padding: 30px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                ...
                <div style="border: 2px solid {pickup_info['color']}; border-radius: 12px; overflow: hidden; margin-bottom: 25px;">
                    ...
                    <div style="padding: 20px;">
                        <h3 style="color: #111827; margin: 0 0 15px 0; font-size: 16px;">ðŸ“ How to Find Your Driver:</h3>
                        <table style="width: 100%; border-collapse: collapse;">
                            {instructions_html}
                        </table>
                    </div>
                ...
                <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 12px; padding: 15px; margin-bottom: 25px; text-align: center;">
                    <p style="color: #92400e; margin: 0; font-size: 16px;">
                        ðŸ‘€ <strong>Look for your name!</strong><br>
            ...
            <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px;">
                <p style="margin: 0;">BookaRide NZ â€¢ Premium Airport Transfers</p>
...
async def startup_event():
    ...
    if not default_admin:
        ...
        logger.info("âœ… Admin password reset and email updated to info@bookaride.co.nz")
    ...
    try:
        ...
        logger.info("âœ… Database indexes created for faster queries")
    except Exception as e:
    ...
    try:
        ...
        logger.info("âœ… Archive indexes created")
    except Exception as e:
    ...
    scheduler.start()
    logger.info("ðŸš€ Scheduler started with all jobs:")
    logger.info("   âœ“ Reminder: 8:00 AM NZ daily (primary)")
    logger.info("   âœ“ Arrival emails: 9:00 AM NZ daily")
    logger.info("   âœ“ Reminder: Hourly backup check")
    logger.info("   âœ“ Abandoned bookings: Every 30 mins")
    logger.info("   âœ“ Auto-sync: Every 5 minutes")
    logger.info("   âœ“ Return alerts: Every 15 minutes")
    logger.info("   âœ“ Daily error check: 6:00 AM NZ daily")
    logger.info("   âœ“ Auto-archive: 2:00 AM NZ daily")
    logger.info("   âœ“ Startup reminder check (running now...)")
